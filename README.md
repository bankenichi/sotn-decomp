...
`matched` count in the table above is *our* work, across five overlays.

The stub count above is `us` only. It excludes `saturn` and the `_psp` trees,
which the queue and the oracle also exclude; counting everything under `src/`
gives 2734, four fifths of it a Saturn port by an external team.
...
- **The `unknown` share is real and is not shrinking on its own.** Those
  records either had their method note overwritten by a build receipt
  (`scheduler.py report` replaces `notes` wholesale) or never carried a note.
  Recoverable going forward; not recoverable for those records. Run
  `match_provenance.py --unknown` for the current list.
- **The categories overlap.** The table counts each match once, by whichever
  step was *decisive*. Counted as contributors instead, the model fleet's
  involvement is much wider than its row suggests: a model draft that the
  permuter drove to zero is counted as `permuter`, deliberately, because
  crediting the model would overstate the fleet.
- **`transplant` is separate from `twin-port` on purpose.** Both move a body
  from a sibling overlay, but a twin-port had its divergences worked out by
  hand while a transplant was placed mechanically by `transplant.py` with the
  substitutions derived from an asm diff. They need different follow-up, so
  they are not pooled.
