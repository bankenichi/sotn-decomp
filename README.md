...
| Code decompiled | **94.1%** (8176 / 8730 functions) across 62 built binaries |
| Queue | 983 records: **425 matched**, 388 todo, 123 escalated, 41 deferred, 6 near |
| `INCLUDE_ASM` stubs left in `src/` | 554 (288 `boss`, 264 `st`, 2 `main`) |
| Automation | 86 Python modules, 29 test suites plus 36 modules with their own `--self-test`, 90 connector tools, 69 diagnostics |

The `matched` count is *our* work, across 18 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 3025, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
**Overall: 94.1% of code decompiled** (8176 of 8730 functions), across 62 built binaries.
