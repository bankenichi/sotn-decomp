...
| Code decompiled | **96.9%** (6384 / 6557 functions) across 44 built binaries |
| Queue | 471 records: **298 matched**, 124 escalated, 43 deferred, 6 near |
| `INCLUDE_ASM` stubs left in `src/` | 176 (94 `boss`, 77 `st`, 3 `servant`, 2 `main`) |
| Automation | 84 Python modules, 28 test suites plus 35 modules with their own `--self-test`, 90 connector tools, 68 diagnostics |

The `matched` count is *our* work, across 6 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 2644, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
| permuter | 17 | 6% | decomp-permuter search reached 0 |
...
- **65 of 298 are unattributed, and that is not shrinking on its own.** 3 were overwritten outright by a build receipt (`scheduler.py report` replaces `notes` wholesale); the other 62 carry no method evidence this tool will accept, which is a weaker claim than saying they are empty. Recoverable going forward, not for these records; `match_provenance.py --unknown` lists them.
- **The categories overlap.** Each match is counted once, by whichever step was DECISIVE. The model fleet is sole author of 56 but contributed to 138; a model draft the permuter drove to zero counts as `permuter`, deliberately, because crediting the model would overstate the fleet.
...
**Overall: 96.9% of code decompiled** (6384 of 6557 functions), across 44 built binaries.
...
| `BOSS/BO0` | 73.5% | 135/186 | harness target |
