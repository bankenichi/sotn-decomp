...
| Code decompiled | **94.1%** (8179 / 8730 functions) across 62 built binaries |
| Queue | 983 records: **425 matched**, 388 todo, 123 escalated, 41 deferred, 6 near |
| `INCLUDE_ASM` stubs left in `src/` | 551 (288 `boss`, 261 `st`, 2 `main`) |
| Automation | 86 Python modules, 29 test suites plus 36 modules with their own `--self-test`, 90 connector tools, 69 diagnostics |

The `matched` count is *our* work, across 18 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 3022, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
|---|---|---|---|
| upstream-harvest | 47 | 11% | upstream had already decompiled it; copied and verified here, **not** produced by this fork |
| shim-segment | 9 | 2% | shared header plus splat segment work |
| shim-header | 55 | 13% | body copied from a shared header |
| transplant | 135 | 32% | transplant.py moved a twin body in mechanically |
...
| model-fleet | 58 | 14% | an OpenCode or llama worker wrote it |
| unknown | 66 | 16% | evidence insufficient; **not** a guess |
...
- **66 of 425 are unattributed, and that is not shrinking on its own.** 3 were overwritten outright by a build receipt (`scheduler.py report` replaces `notes` wholesale); the other 63 carry no method evidence this tool will accept, which is a weaker claim than saying they are empty. Recoverable going forward, not for these records; `match_provenance.py --unknown` lists them.
- **The categories overlap.** Each match is counted once, by whichever step was DECISIVE. The model fleet is sole author of 58 but contributed to 143; a model draft the permuter drove to zero counts as `permuter`, deliberately, because crediting the model would overstate the fleet.
...
**Overall: 94.1% of code decompiled** (8179 of 8730 functions), across 62 built binaries.
