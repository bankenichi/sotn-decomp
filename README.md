...
The `matched` count is *our* work, across 6 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 2648, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
| permuter | 15 | 5% | decomp-permuter search reached 0 |
...
- **65 of 294 are unattributed, and that is not shrinking on its own.** 3 were overwritten outright by a build receipt (`scheduler.py report` replaces `notes` wholesale); the other 62 carry no method evidence this tool will accept, which is a weaker claim than saying they are empty. Recoverable going forward, not for these records; `match_provenance.py --unknown` lists them.
- **The categories overlap.** Each match is counted once, by whichever step was DECISIVE. The model fleet is sole author of 55 but contributed to 135; a model draft the permuter drove to zero counts as `permuter`, deliberately, because crediting the model would overstate the fleet.
...
**Overall: 96.9% of code decompiled** (6381 of 6558 functions), across 44 built binaries.
...
| `ST/RNO0` | 82.8% | 157/191 | harness target |
| `BOSS/BO6` | 74.4% | 193/237 | harness target |
| `BOSS/BO0` | 73.2% | 134/186 | harness target |
