...
| Code decompiled | **96.9%** (6381 / 6557 functions) across 44 built binaries |
| Queue | 471 records: **295 matched**, 12 todo, 114 escalated, 43 deferred, 7 near |
| `INCLUDE_ASM` stubs left in `src/` | 179 (96 `boss`, 78 `st`, 3 `servant`, 2 `main`) |
| Automation | 84 Python modules, 28 test suites plus 35 modules with their own `--self-test`, 90 connector tools, 68 diagnostics |

The `matched` count is *our* work, across 6 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 2647, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
**Overall: 96.9% of code decompiled** (6381 of 6557 functions), across 44 built binaries.
