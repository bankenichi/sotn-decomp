...
| Code decompiled | **94.0%** (8171 / 8734 functions) across 62 built binaries |
| Queue | 983 records: **420 matched**, 393 todo, 122 escalated, 42 deferred, 6 near |
| `INCLUDE_ASM` stubs left in `src/` | 563 (292 `boss`, 269 `st`, 2 `main`) |
| Automation | 86 Python modules, 29 test suites plus 36 modules with their own `--self-test`, 90 connector tools, 69 diagnostics |

The `matched` count is *our* work, across 18 overlays. The stub count is `us` only: it excludes `saturn` and the `_psp` trees, which the queue and the oracle also exclude. Counting every `.c` under `src/` instead gives 3034, most of it a Saturn port by an external team.
<!-- STATUS:END -->
...
| unknown | 65 | 15% | evidence insufficient; **not** a guess |
...
**Overall: 94.0% of code decompiled** (8171 of 8734 functions), across 62 built binaries.
