...
| 299 | **open** | Concrete programmatic chaining gap: factory permuter providers freeze a renderer or preserved-file seed before deterministic lanes execute. Later task ancestry lists earlier candidates, but `PermuterLaneProvider._input_for` still returns that original frozen seed. A new compiling transplant therefore does not automatically become the next mutation input. Add a durable task-bound seed handoff selected from actual evaluated candidates, including exact source and parent measurement provenance, provider reconstruction and recovery validation. Listing candidates as task parents is not proof they were consumed. |
