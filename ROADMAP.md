...
| 287 | **in progress** | Instrumentation review IR5: repair concrete permuter resume. Start and resume share immutable request/state paths but carry different bytes, so materialization refuses; replaying the original seed also does not restore search progress. Preserve session evidence, use phase-specific work state, and prove interrupted execution continues without duplicate evaluations. Phase-specific scratch and durable per-source scores passed real start/resume and no-recompile replay in `run_automation-104407-26`. Crash recovery across incomplete provider handoffs and complete interrupted/uninterrupted sequence comparison remain open. |
