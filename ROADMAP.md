...
| 239 | **partial, 1/3** | Close the complete three-record near pool in deterministic order: `us:ST/RCEN:func_801904B8`, `us:ST/RNO0:func_us_801C7F24`, then `us:BOSS/BO0:func_us_801B001C`. `func_801904B8` is done at 81/81: the same-basename CEN sibling was unhidden by an exact-path donor fix, asm evidence adapted `dy -= 32` to RCEN's `dy += 0x20`, and the remaining signed `dy >= 0x101` zero fallback produced isolated score 0 before the verified build. The exact whole source is immutable v0005. Continue with `func_us_801C7F24`, then `func_us_801B001C`; do not request new model generations until each preserved body is structurally exhausted |
