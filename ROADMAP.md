...
| # | status | task and outcome |
|---|---|---|
| 98 | **open** | Re-baseline Stage 6 against the live queue before execution; the 2026-08-10 counts no longer describe the 62 todo and 95 escalated records |
...
| 204 | **open** | Audit priority 2: review fork-generated C for the reported cosmetic regressions, including stray semicolons, duplicate externs, missing fallthrough annotations, trailing whitespace, and retained worker-injection boilerplate; preserve every load-bearing shape |
...
| 206 | **open** | Audit priority 2: design a post-match lint for redundant externs, shared-macro shadowing, suspicious scalar types, stray null statements, missing fallthrough annotations, trailing whitespace, and worker declaration boilerplate; start as a report-only gate |
| 207 | **open** | Audit priority 3: add `test_permuter_import_parser.py` and `test_progress_detection.py` to the operational test guidance beside the code paths they protect |
| 208 | **open** | Audit priority 3: assess whether stale-workdir detection, journal handling, and seed management should be extracted from `permuter_supervisor.py`; preserve current behavior and archive any superseded design |
| 209 | **open** | Audit priority 3: define a review pass for model-generated matched functions whose comments record provenance but not the compiler constraint or assembly reasoning that makes an unusual C shape necessary |
