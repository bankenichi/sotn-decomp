...
| 274 | **open** | OWNER OVERRIDE: implement the instrumented search system specified by `docs/superpowers/specs/2026-08-26-instrumented-search-system-design.md`, `automation/search-ledger.schema.json` and the paired implementation plan before resuming receipt execution. This temporarily supersedes the execution priority of #257 through #273 because its shared ledger, score vectors, mutation provenance, subset orchestration, recovery, recombination and lane receipts may satisfy or re-scope those tasks. All earlier records remain in scope and will be reconciled rather than deleted. The baseline before documentation was 113/113 with the pre-existing dirty work preserved. The `automation/instrumented-search` checkpoint now carries the typed ledger/archive/schema, deterministic subset and lane runtime, scorer/permuter instrumentation, mutation and frontier machinery, strict coordinator/recovery replay validation, durable idempotent oracle handoff, compiler idiom and draft-landed evidence foundations, connector/selftest controls, and reviewed follow-on corpus, donor-index, m2c-matrix and weight-tuner plans. Task 8.2 is complete: explicit legacy/instrumented ownership now drives coordinator-owned typed tasks, deterministic multi-candidate fan-out, durable landing receipts, and hash-bound stop/resume replay without duplicate scheduling or oracle calls. Task 8.3 is complete. A one-record read-only plan across all 11 deterministic lanes canonicalized identically under reversed input, then the public start refusal exposed that no canonical manifest producer was reachable. That diagnosis is retained as the reason for adding the seventh bounded surface, `search_create_instrumented`. Creation now freezes the exact todo subset plus queue, source, target, compiler, configuration, schema, tool and lane-input evidence; publishes an immutable intent, evidence index and manifest with crash-safe exact retries; enforces runtime identity before task start; and budgets the base and at most two child tasks per base under the shared 4096-task cap. The connector restart and bounded multi-record live shadow remain the next runtime boundary, so this record stays open. |
