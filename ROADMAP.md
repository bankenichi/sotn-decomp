...
| 288 | **in progress** | Instrumentation review IR6: replace candidate iteration numbers synthesized from sorted output-directory enumeration with actual structured vendor event provenance. Preserve unknown historical chronology explicitly and add nonconsecutive-iteration attribution coverage. Replaced directory-name chronology with content-addressed per-evaluation source, mutation, score, object and iteration records, plus completed-operation/conflict evidence. Real executor and five-provider public acceptance passed in `run_automation-153500-26`; stronger tamper and sparse-improvement attribution coverage remains open. |
