...
| 220 | **open** | Extend `ext_demand.py` and the worker preflight to recognize raw Entity-base offset expressions such as `&((u8*)g_CurrentEntity)[0x90]`, map them to existing Ext variants when possible, and flag access-width or wrong-member mismatches. During #219 the current analyzer incorrectly reported that no generated file touched ext even though v0004 contained exactly that raw access, forcing a manual lookup that existing layout data could have answered |
