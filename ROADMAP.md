...
| Decompiled | **96.9%**, 6379/6558 functions; 182 US `INCLUDE_ASM` stubs remain |
| Queue | 471 records: 292 matched, 34 todo, 102 escalated, 42 deferred, 1 near |
| Provenance | upstream-harvest 44, shim-segment 9, shim-header 55, transplant 18, twin-port 29, permuter 13, claude-manual 4, model-fleet 55, unknown 65 |
...
| 217 | **open** | Resolve exact-width access to named `Entity` subobjects before retrying `us:ST/RCEN:func_801904B8`. The #101 fleet proved that this function intentionally writes individual bytes and halfwords inside `velocityX`, `velocityY`, `zPriority` and `params`; the worker alternated between forbidden raw offsets, aggregate assignments with the wrong access width and nonexistent alias members. Extend the authoritative layout or add a proven named access idiom, then teach the context and quality gate that representation |
| 218 | **open** | Harden supporting-structure context before retrying `us:ST/RNO0:func_us_801CFC98`. The fleet found a plausible named-member form for the upper halves of `posX` and `posY`, but then invented `Collider.hit`; the real project member is `effects`, so the final candidate failed compilation after four attempts. Supply exact reachable struct members to generation and reject absent supporting-struct members before a build |
| 219 | **open** | Advance the preserved `us:BOSS/BO0:func_us_801B1C60` near seed after resolving entity offset `0x90`. Three fleet generations compiled and linked; the post-fleet corpus gate published declaration-complete v0004 as the current seed without changing its body. The ext variants supplied to the worker expose no named member at that address, so the candidate retains a byte-pointer access. Prove the field or intentional byte view first, then import through the supervisor and use the permuter |
