...
| 202 | **open** | Audit priority 1: verify `src/st/rchi/e_gaibon.c` local definitions for `ENTITY_DEFAULT`, `ENTITY_ROTATE`, `ENTITY_SCALEX`, `FLAG_DEAD`, and `PAL_UNK_1B6` against the shared headers and target assembly; remove only proven shadow copies and preserve codegen |
| 203 | **open** | Audit priority 1: verify the `unsigned long parentAnimFrame` type and cast in `src/st/rchi/e_gaibon.c` against the real struct layout and assembly before deciding whether `u16` or `s16` is correct |
...
| 205 | **open** | Audit priority 2: verify and explain the load-bearing `do { ... } while (0)` in `src/boss/bo0/3053C.c:func_us_801B1E5C` with an in-source `CODEGEN:` comment if the frame-shape claim is confirmed |
