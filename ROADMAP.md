...
| 241 | **open** | Resolve all 59 remaining symbol escalations. #240 proved the former 21-record estimate was based on a lossy taxonomy: 32 records still have no unresolved identifier under the old report shape, but Ext diagnostics, receiver-type ambiguity, missing preserved bodies and unverified flat paths prevent automatic execution. Continue from the versioned candidates and isolated compile receipts produced by #240. Use overlay-local maps, `ext_demand.py`, `member_types.py`, caller context and assembly widths; never borrow raw-address names across overlays. Reclassify each record only after the unresolved list is empty or explicitly evidenced |
