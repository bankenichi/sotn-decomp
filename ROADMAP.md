...
| 328 | **done** | Muse / Zen Responses fleet logged effort=none and then 0 chars after ~100s. Dashboard effort 0 (UI low) was dropped by `any(eff)` so REASONING_EFFORT was omitted and worker_direct defaulted to none; `_responses_generate` also hid incomplete max_output_tokens starvation as an empty parse. Dashboard now always sends per-worker effort (0=low, 1=none, 2=xhigh). Muse Responses always requests reasoning headroom with a 32k floor, logs status/incomplete_details/usage, and captures output_text parts. Offline parse fixtures in test_responses_parse.py; dashboard self-test updated. No src/ change and no make_build. |
