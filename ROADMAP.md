...
| 283 | **open** | Instrumentation review IR1: finish factory production of immutable provider state and concrete tool/input bindings for the nine generated, idiom, permuter and model lanes. Reconstruction consumers exist but the factory currently supplies no provider-state artifact. Complete the existing production plan's Task 7 public acceptance and adapt its callback-injecting fixtures without weakening the no-override rule. |
| 284 | **open** | Instrumentation review IR2: connect materialized search candidates to explicit, factory-bound isolated evaluation tasks and evaluator provenance. Separate discovery completion from evaluated exhaustion; record unexamined candidates and replace hash-order selection presented as best-candidate evidence with a measured selection policy. Preserve indexed discovery's read-only boundary and existing full-oracle authority. |
| 285 | **open** | Instrumentation review IR3: repair the permuter process-result argument order and replace the newly required host `cc` wrapper with the repository-owned target compiler recipe. `_run_process` returns returncode before output while `_parse_output` expects output first; the wrapper also lacks object-only target compilation. Prove one complete executor invocation against a known target fixture before acceptance. |
| 286 | **open** | Instrumentation review IR4: implement the actual targeted, recombination and ddmin operations. Current executor lane names only choose difflib or levenshtein scoring for the same vendor command. Bind strategy and scorer separately and require behavioral strategy fixtures. |
| 287 | **open** | Instrumentation review IR5: repair concrete permuter resume. Start and resume share immutable request/state paths but carry different bytes, so materialization refuses; replaying the original seed also does not restore search progress. Preserve session evidence, use phase-specific work state, and prove interrupted execution continues without duplicate evaluations. |
| 288 | **open** | Instrumentation review IR6: replace candidate iteration numbers synthesized from sorted output-directory enumeration with actual structured vendor event provenance. Preserve unknown historical chronology explicitly and add nonconsecutive-iteration attribution coverage. |
| 289 | **open** | Instrumentation review IR7: correct or refuse target-renderer OR-immediate translation and unproven stack-load/store elision. The supported `ori v0,a0,1` form currently renders addition; add semantic counterexamples before treating emitted drafts as target-derived evidence. |
| 290 | **open** | Instrumentation review recommendation: retain sanitized model response envelopes, actual provider identity, token usage, finish reason, timing and precise transport/error taxonomy. Current proposal transport drops those measurements, preventing reliable cost, empty-response and failure comparisons. Keep operational timing separate from deterministic replay identity. |
...
| 292 | **open** | Instrumentation review recommendation: expose the recipient/lane funnel with separate applicability, execution, generation, deduplication, evaluation, improvement, oracle and knowledge-reuse counts. Finish the existing production acceptance rather than adding speculative lanes; retain data-segment instrumentation under #277 for subsequent scope. |
