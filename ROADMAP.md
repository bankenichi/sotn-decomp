...
| 281 | **open** | Complete the evidence-corpus and donor-index correction and query tranche under `docs/superpowers/plans/2026-08-30-evidence-correction-parallelization.md`. This explicitly retracts #280's blanket statement that R1 through R18 were all resolved: R6 documentation drift survived that landing and is now corrected. Root accepted R19 through R44 with final focused proof corpus 60/60, donor-index 27/27, patterns 38/38 and lanes 27/27, plus consolidated job `run_automation-154506-6216` at 84/84. R34 is accepted with supervisor 39/39 and donor-index 29/29. The R24/R35 donor query core is accepted through R49: durable archives and expected binding are mandatory, stale semantics use the caller expectation, replay recomputes against verified evidence, renderer-safe claims exclude provenance, and final job `run_automation-163329-6216` passed 18/18. The R30 indexed adapter is accepted through R53: equivalent claims are deduplicated before rendering, exact claim and refusal identities remain durable lane inputs, stale is distinct from exhausted search, target artifacts are fully validated, and final focused job `run_automation-195951-12593` passed adapter 12/12 and lanes 28/28. The remaining step is the one final focused matrix and consolidated suite, followed by explicit-path commits, the fresh 113-artifact oracle and origin push gate. |
