...
| 257 | **open** | RESCOPED after #256. Exhaust all 32 `tiny-closed` records in `automation/mechanical-waves.us.json` in instruction-count order, batching same translation units under the journaled oracle and evidence-backing every rejection before advancing. Partial 2026-08-25: the first record, RNZ1 `func_us_801ABDC8`, was derived as two `s32` decrements twenty bytes apart, reached isolated score zero, and matched RNZ1 plus the complete 113-artifact oracle through the journaled landing path. Its immutable candidate and queue proof are preserved; 31 original wave records remain to disposition. |
