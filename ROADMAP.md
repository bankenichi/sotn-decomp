...
| `BOSS/RBO4` | 7 | 6 | 0 | 1 | 0 | 0 |
| `BOSS/RBO6` | 35 | 33 | 0 | 2 | 0 | 9 |
| `BOSS/RBO7` | 8 | 7 | 0 | 1 | 0 | 1 |
| `BOSS/RBO8` | 17 | 16 | 0 | 1 | 0 | 7 |
| `MAIN` | 2 | 0 | 0 | 2 | 0 | 0 |
| `ST/RCEN` | 14 | 0 | 0 | 14 | 0 | 9 |
| `ST/RCHI` | 9 | 0 | 0 | 7 | 2 | 6 |
| `ST/RDAI` | 18 | 0 | 0 | 15 | 3 | 0 |
| `ST/RLIB` | 20 | 19 | 0 | 1 | 0 | 2 |
| `ST/RNO0` | 31 | 1 | 2 | 18 | 10 | 188 |
| `ST/RNO1` | 23 | 23 | 0 | 0 | 0 | 19 |
| `ST/RNO2` | 32 | 30 | 0 | 2 | 0 | 16 |
| `ST/RNO4` | 61 | 60 | 0 | 1 | 0 | 23 |
| `ST/RNZ1` | 44 | 40 | 0 | 3 | 1 | 21 |
| `ST/SEL` | 1 | 0 | 0 | 1 | 0 | 0 |
...
US is the sole work target. Other versions supply donor/reference evidence;
indexing or harvesting them does not authorize decompiling or landing them.
...
and scorer weight tuning (#304) are immediate priorities alongside cross-platform
function matching (#302) and evaluated-candidate chaining (#299). Their required
evidence collection is part of implementation, not a reason to park them behind
whole-queue exhaustion. Model and fleet activation remain deferred under #293.
This supersedes the exclusive execution order below for harness implementation;
the older queue waves and deferred records retain their scope. See #303.

Historical queue execution order, retained as evidence:
...
| 251 | done | Extend upstream discovery beyond merged `upstream/master` to inspect explicitly supplied open-PR heads and pinned commits through a read-only, provenance-preserving lane. Report exact current queue overlaps, already-matched functions and binaries absent from this fork's current build scope. RETRACTION: the initial 2026-08-24 audit treated PRs #3529 and #3617 as out of scope; #253 confirms their RBO1, RBO7 and RBO8 binaries belong in the intended scope. Commit 91498064 and PRs #3617 and #3529 remain the acceptance corpus. [September 14 completion: both PRs are merged upstream (#3617 as 9cd0decb, head 8af8d636; #3529 as 233f9b84, head fd7ef91e), so discovery is a `--ref` lane on locally available refs: `upstream_harvest.py --ref` compares any explicit ref with per-ref caches, `--verify` ref resolution, and unchanged default behavior. Unmatched overlaps: 194 harvestable at 91498064, 198 at 9cd0decb (+4 RBO8), 225 at 233f9b84, 226 at upstream/master d5e5165b6; every row falls in the 24 tracked overlays, so no acceptance-corpus binary is absent from build scope (RBO1/RBO7/RBO8 vendored under #253). Already-matched at d5e5165b6: 230 of our 434 matched are also upstream-decompiled; we name fewer fields in 0, carry ILLEGAL worse in 1 (18 vs 0) and raw D_ worse in 3 rows (all EntityJackOBonesDeathParts), with 1 upgradable field name. Fork heads 8af8d636/fd7ef91e are not fetched locally and are covered via their merge commits; fetching new open-PR heads (`git fetch upstream refs/pull/N/head`) remains operator WSL work until an open-PR corpus needs it, since the connector has no refspec surface. No src, queue, or candidate writes were part of this lane.] [September 14 compare-matched follow-through: the comparison itself needed two correctness fixes before its findings could be trusted. Upstream-only Saturn twins are now skipped (Saturn Entity layout differs: attacker pointer at 0xB4 vs PSX 0xB8, which produced a false upgradable field on HitDetection), and both sides resolve through the record's own overlay (a lexical hit list had compared Saturn's port against upstream PSX for InitRoomEntities). With honest attribution the remaining findings were fixed and verified: ET_801C03E8 adopted in func_us_801C03E8, Jack O'Bones/rotspeeds/Paranthropus/Slogra/Gaibon/Karasuman/Spikes/Imp/KillerFish/Cloak tables and EInits named across rno0/rno1/rno2/rno3/rno4/rchi/rnz1, final compare 0 worse in every class at 113/113.] |
...
| 267 | **open** | OWNER OVERRIDE: the current live `todo` population is the exclusive execution lane until `queue_stats` reports `todo: 0`. Execute #257 through #264 in order and do not advance #242 through #252, #265, or any non-todo record before that boundary. The deferred tasks remain open with all prior evidence intact; refresh their live populations only after todo exhaustion. [September 7 amendment: #303 supersedes the exclusive queue execution order for immediate harness implementation. Queue records and eventual wave execution remain in scope.] |
...
| 274 | done | OWNER OVERRIDE: implement the instrumented search system specified by `docs/superpowers/specs/2026-08-26-instrumented-search-system-design.md`, `automation/search-ledger.schema.json` and the paired implementation plan before resuming receipt execution. This temporarily superseded the execution priority of #257 through #273 because its shared ledger, score vectors, mutation provenance, subset orchestration, recovery, recombination and lane receipts could satisfy or re-scope those tasks. All earlier records remain in scope and will be reconciled rather than deleted. The baseline before documentation was 113/113 with the pre-existing dirty work preserved. The `automation/instrumented-search` branch now carries the typed ledger/archive/schema, deterministic subset and lane runtime, scorer/permuter instrumentation, mutation and frontier machinery, strict coordinator/recovery replay validation, durable idempotent oracle handoff, compiler idiom and draft-landed evidence foundations, connector/selftest controls, and reviewed follow-on corpus, donor-index, m2c-matrix and weight-tuner plans. Task 8.2 is complete: explicit legacy/instrumented ownership drives coordinator-owned typed tasks, deterministic multi-candidate fan-out, durable landing receipts, and hash-bound stop/resume replay without duplicate scheduling or oracle calls. Task 8.3 is complete. A one-record read-only plan across all 11 deterministic lanes canonicalized identically under reversed input, then the public start refusal exposed that no canonical manifest producer was reachable. That diagnosis is retained as the reason for adding the seventh bounded surface, `search_create_instrumented`. Creation freezes the exact todo subset plus queue, source, target, compiler, configuration, schema, tool and lane-input evidence; publishes an immutable intent, evidence index and manifest with crash-safe exact retries; enforces runtime identity before task start; and budgets the base plus at most two child tasks per base under the shared 4096-task cap. The first three-record live create exposed case-alias duplication on the Windows-backed filesystem and was corrected with filesystem-identity deduplication while preserving genuine ambiguity refusal. The first live execution then exposed four preserved candidates under a two-child allowance; the full outcome remains archived and validated while canonical selection bounds child materialization. The corrected `shadow-three-20260829-b` run completed 35 tasks with none incomplete, retained all three queue records as unclaimed `todo`, and verified a 142-event ledger through sequence 141 at `sha256:e14de16df6fcf80ff93574fec987f488c55e49958095085f59097ce5009eeea1`. This closes the instrumented-search implementation boundary; #257 through #273 resume under the new receipt system. |
| 275 | done | Fixed generated candidate and rejection artifacts accumulating trailing spaces, whitespace-only lines and multiple final blank lines. Root cause was the shared `publish_versioned_artifact` path encoding caller text byte-for-byte into both immutable history and the stable view. The publisher now canonicalizes horizontal trailing whitespace and exactly one final newline before either write, and its self-test proves both outputs receive identical normalized bytes. Existing affected artifacts in this checkpoint were normalized through the same policy. |
| 276 | done | Fixed the consolidated self-test suite rewriting tracked `automation/opencode/opencode.json`. Root cause was the connector-surface fixture calling the real `fleet_stop`, whose production contract intentionally refreshes the live Zen catalogue for the next run. The fixture now isolates that external refresh while retaining the real reclaim and journal-replay path, and byte-for-byte regressions prove both fleet-stop cases preserve the tracked catalogue. |
| 277 | done | Design and implement a data-segment search subsystem after #274 validates the function-search ledger. Preserve byte-serialization candidate identity, data-specific score vectors, target-range ownership and exact oracle routing without generalizing `CandidateRecord` into an untested function/data union during the current delivery. This records the accepted avenue from the 2026-08-27 instrumentation supplement; it is deferred, not removed from scope. [September 7 owner amendment: promoted from deferred to immediate implementation priority by #303; distinct data records and exact byte/range evidence remain required.] [September 7 implementation: data_search.py now freezes configured binaries, verifies calibrated byte/range candidates, and exposes preparation, dependency closure and journaled full-oracle landing. RNO1 red-door discovery found one exact 24-byte range from 48 captured configs (14 exclusions). The corrected closure-v2 attempt compiled and linked but failed the checksum; original source/config/symbols and queue status were restored with the complete baseline verified. Generic non-US preparation, wider data families and a successful data landing remain open; no function match is claimed.] [September 8 RETRACTION: the non-US preparation item above was a scope error, repeated in the lead status report. US is the only decompilation/preparation/landing target. HD, PSPEU and Saturn are donor corpora only. Remaining preparation work concerns US boss overlays, additional US data families and adaptation of foreign donor evidence into US context; successful landing is deferred to the later execution phase.] [September 8 donor-direction implementation: new data-search requests reject non-US targets before acquiring ownership. Discovery now captures checksum-bound foreign splat and Saturn-directory donors for US byte calibration and frozen replay, retaining unverifiable donor configs as exclusions. Existing US preparation/landing scope is unchanged; foreign targets cannot be created.] [September 14 implementation: boss-overlay and rodata search proven by fixture without production changes. `test_data_search.py` gains a `bobo0` boss-overlay target test (discovery glob treats boss configs like stage targets; calibrated range eligible and frozen-replay verified) and a `rodata` section end-to-end test, 15/15 passing. Foreign-donor adaptation into US context was already covered by the hd/pspeu donor test. Successful data landing stays deferred to the later execution phase per the owner instruction; no function match is claimed.] |
| 278 | done | Landed the 2026-08-29 eval follow-up batch. Pushed the instrumented-search boundary: origin gained the fanout-hardening commit, the evidence-plan docs commit, and the `queue.20260830-035336` checkpoint snapshot (983 records, `433 matched`), after the fresh pre-push gate returned selftests 82/82 and the oracle 113/113 with clean invariants on both branches. Retracted three eval findings against measured evidence: the live queue is authoritative at 433 matched with no re-opened records (the stale August 27 snapshot simply predated 8 landings); the Git-for-Windows junction and CRLF phantoms do not exist in the canonical WSL environment where the tree is clean; and all 12 Windows-side suite failures fail to reproduce canonically. Exported the integration gate surface the corpus and tuner plans consume as `IntegrationGateReceipt`: `automation/search_supervisor.py` now owns the typed receipt with its archived-artifact identity, `IntegrationGateError`, the reserved `EVALUATOR_TOOL_KEY`, `archive_integration_gate` production from factory-bound coordinator and measured connector identities, and the canonical `validate_integration_gate`/`load_integration_gate` pair, with focused supervisor tests covering round-trip, changed-identity, corrupt-artifact and missing-binding refusals. [Issuance superseded by #280: lead review R1 found manifest-based issuance could not prove run completion; gate issuance was reworked to run-root-only durable proof.] |
| 279 | done | Started the evidence-corpus/donor-index plan ahead of the mechanical waves per owner sequencing, so the corpus and donor index become part of the systematic work instead of following it. Task 0 landed at the plan's commit boundary: `automation/search_evidence_corpus.py` consumes the canonical Task 8.2 integration receipt under the descriptive `IntegrationGateReceipt`/`IntegrationGateError` names, calls `validate_integration_gate` exactly once before reading any evidence, and publishes an immutable `CorpusGeneration` whose artifact retains the complete receipt payload including the archived receipt artifact reference; the focused suite covers missing, changed and corrupt gate refusals plus full provenance retention. Also committed the vendored `tools/decomp-permuter/test/compile.sh` with mode 100755: the earlier checkout round-trip had rewritten it to the committed 644 and `core.filemode=false` hid the loss until the vendored suite hit PermissionError, so the mode is now the index's own truth. [Corrected by #280: lead review R7 found the first generation record accepted copied provenance that could disagree with its embedded gate; the binding is now enforced, and the Task 2 boundary carried lead-directed corrections R10 through R18.] |
| 280 | done | Parallel three-lane correction tranche under the shared coordination ledger, landed by the acting lead. Lead review R1 through R18 resolved: gate issuance is now run-root-only durable proof (factory archive verification, full ledger recovery, base-task coverage identity, smoke versus multi-record kinds, terminal non-resumable completion stop, and validation that re-derives the expected gate from disk instead of trusting self-consistency), the supervisor mints the gate itself when a factory-created run completes so the recorded connector hash belongs to the executing process, successful supervisor runs append exactly one terminal `RunStop(reason="completed")` with deterministic replay, lesson citations reapply every constructor invariant on verification and bind the section 2 absent-andi rule to its exact anchor with strict source metadata checks, scorer taxonomy forgery surfaces domain errors only, `CorpusGeneration` binds its generation id, artifact bytes and copied provenance to the embedded gate, completed-lineage projection is shared, identity-sorted and deduplicated, excludes diagnostic ledgers from recommendations, runs the canonical coordinator semantic validation on every mined ledger, and both public records reject forged shapes as typed domain errors. The connector commit-time documentation sync defers around unrelated unstaged or untracked worktree state instead of staging generated output from it, `fs_write` verifies its post-replace bytes and retries post-replace visibility lag with bounded backoff, pure libraries cannot become automation actions, and the committed vendored compile script is executable in the index. Workers released all six implementation files after focused proof; the acting lead re-ran the released suites on the stable tree (supervisor, corpus, factory all green) before consolidated validation. |
| 281 | done | Completed the evidence-corpus and donor-index correction and query tranche under `docs/superpowers/plans/2026-08-30-evidence-correction-parallelization.md`. This explicitly retracts #280's blanket statement that R1 through R18 were all resolved: R6 documentation drift survived that landing and is now corrected. Root accepted R19 through R44 with focused proof corpus 60/60, donor-index 27/27, patterns 38/38 and lanes 27/27. R34 is accepted with supervisor 39/39 and donor-index 29/29. The R24/R35 donor query core is accepted through R49 with mandatory durable archives, caller-relative stale semantics, verified deterministic replay and provenance-free semantic claims. The R30 indexed adapter is accepted through R53 with deduplicated renderer claims, exact durable claim and refusal identities, distinct stale disposition and canonical target artifacts. The final seven-suite matrix passed 4/4 plus 3/3 in disjoint jobs, connector R54 made both new suites directly callable while keeping their libraries unlisted, commit `a866e5d556135af021ce3d6019606c379418bdca` synchronized all managed documents, and consolidated job `run_automation-201332-12520` passed 86/86. |
| 282 | done | Reviewed the current instrumentation working tree at d22c3e61f against the fork's harness and knowledge-base objective. The canonical suite passed 98/101; the existing one-record integration gate verified 27 events but produced six inapplicable receipts and no candidate/evaluation events. Preserved seven implementation findings and six instrumentation recommendations in `docs/audit/2026-09-04-automation-instrumentation-review.md`; no implementation fixes or live provider dispatch were performed. Follow-ups #283 through #292 remain open. |
| 283 | **in progress** | Instrumentation review IR1: finish factory production of immutable provider state and concrete tool/input bindings for the nine generated, idiom, permuter and model lanes. Reconstruction consumers exist but the factory currently supplies no provider-state artifact. Complete the existing production plan's Task 7 public acceptance and adapt its callback-injecting fixtures without weakening the no-override rule. Factory now publishes indexed immutable state for bounded synthesis, four concrete permuter strategies and two lazy model providers; public five-provider create/start/recovery passed in `run_automation-153500-26`, with model public acceptance still running. The idiom and qualified m2c factory inputs remain open. [September 7 reconciliation: the earlier assertion that the factory supplies no provider state is superseded by its implementation. Idiom input binding exists under #293. Qualified m2c publication, factory admission and live execution remain open; model admission remains deferred.] [September 14 scoping, implementation deferred to keep the factory capture path honest: root cause narrowed to three admission points, all verified present-but-refusing rather than missing. `search_run_factory._LANE_MODULES` has no `m2c_ensemble` entry; `search_provider_factory.prepare_provider_inputs` refuses it via `IMPLEMENTED_LANES` ("requires qualified runtime inputs"); the provider side already exists (`LANE_PROVIDER_REGISTRY["m2c_ensemble"]`, `build_m2c_ensemble_provider`, `GeneratedLaneProvider`). Admission needs a matrix-receipt capture input, a `publish_provider_state` m2c branch, the `_LANE_MODULES` entry, audit expectation updates, and a fixture factory execution. Wiring the entry without the capture path would turn the production audit green while creation fails at runtime, so the entry stays out until the full path lands with fixture proof.] |
| 284 | **done** | Instrumentation review IR2: connect materialized search candidates to explicit, factory-bound isolated evaluation tasks and evaluator provenance. Separate discovery completion from evaluated exhaustion; record unexamined candidates and replace hash-order selection presented as best-candidate evidence with a measured selection policy. Preserve indexed discovery's read-only boundary and existing full-oracle authority. Implemented real PSX isolated evaluation, selected-function scoring, durable result reuse, transitive recovery checks and public factory/start candidate measurement in job `run_automation-024514-85`; measured selection and unexamined dispositions remain open. Added actual best-measured selection, per-candidate unexamined dispositions, source-local durable score reuse and convergent-lane provenance retention. Focused evaluator, supervisor and factory proof passed in `run_automation-104652-26`; combined provider/recovery proof passed in `run_automation-153500-26`. Consolidated recovery/report validation remains open. [September 7 closure: measured selection, unexamined dispositions and consolidated recovery/report checks were implemented and exercised by the subsequent #296/#300 landing batch. Knowledge-transfer and full funnel acceptance remain separately open under #291/#292.] |
| 285 | **done** | Instrumentation review IR3: repair the permuter process-result argument order and replace the newly required host `cc` wrapper with the repository-owned target compiler recipe. `_run_process` returns returncode before output while `_parse_output` expects output first; the wrapper also lacks object-only target compilation. Prove one complete executor invocation against a known target fixture before acceptance. Complete real PSX executor invocation passed in `run_automation-104001-26`; start/resume and durable score reuse passed in `run_automation-104407-26`. The repository wrapper now uses the existing target pipeline, rejects compiler drift, and accepts binary ELF target bytes containing NULs. The earlier host-compiler and process tuple-order diagnoses were reproduced before correction. |
| 286 | **done** | Instrumentation review IR4: implement the actual targeted, recombination and ddmin operations. Current executor lane names only choose difflib or levenshtein scoring for the same vendor command. Bind strategy and scorer separately and require behavioral strategy fixtures. Concrete targeted pass restriction, two-donor grouped-patch recombination and measured hunk minimization now execute in the structured worker. Behavioral strategy tests passed in `run_automation-104234-26`; the public factory path retained real recombination conflicts in `run_automation-153500-26`. Consolidated acceptance remains open. [September 7 closure: the implemented targeted, recombination and ddmin behavior passed the subsequent consolidated gates recorded under #296/#300. Incomplete-handoff recovery remains #287; no new strategy implementation is claimed here.] |
| 287 | done | Instrumentation review IR5: repair concrete permuter resume. Start and resume share immutable request/state paths but carry different bytes, so materialization refuses; replaying the original seed also does not restore search progress. Preserve session evidence, use phase-specific work state, and prove interrupted execution continues without duplicate evaluations. Phase-specific scratch and durable per-source scores passed real start/resume and no-recompile replay in `run_automation-104407-26`. Crash recovery across incomplete provider handoffs and interrupted/uninterrupted sequence comparison closed 2026-09-14: new `test_search_seed_handoff.py` proves an incomplete handoff rebinds identically with no duplicate, refuses outside-subset recipients with `SeedHandoffError` and tampered archives with `ArchiveError`; `bind_task_provider` no longer leaks `KeyError`; and a resume-sequence test proves interrupted execution matches the uninterrupted measurement with prefix cache reuse and no duplicate evaluations. |
| 288 | done | Instrumentation review IR6: replace candidate iteration numbers synthesized from sorted output-directory enumeration with actual structured vendor event provenance. Preserve unknown historical chronology explicitly and add nonconsecutive-iteration attribution coverage. Replaced directory-name chronology with content-addressed per-evaluation source, mutation, score, object and iteration records, plus completed-operation/conflict evidence. Real executor and five-provider public acceptance passed in `run_automation-153500-26`. Tamper and sparse-improvement attribution closed 2026-09-14: `load_events` refuses filename/content-hash mismatches, `best_so_far_improvements` attributes sparse best-so-far improvements with artifact provenance, and 9 new focused tests pass in `test_search_permuter_worker.py` (15/15) with executor 20/20 and recovery 22/22 unaffected. |
| 289 | **done** | Instrumentation review IR7: fixed OR rendering and refused unproven stack data traffic; counterexamples failed before the edit and the 13 renderer tests passed in job `run_automation-213737-85`. Historical diagnosis: correct or refuse target-renderer OR-immediate translation and unproven stack-load/store elision. The supported `ori v0,a0,1` form currently renders addition; add semantic counterexamples before treating emitted drafts as target-derived evidence. |
| 290 | **done** | Instrumentation review recommendation: retain sanitized model response envelopes, actual provider identity, token usage, finish reason, timing and precise transport/error taxonomy. Current proposal transport drops those measurements, preventing reliable cost, empty-response and failure comparisons. Keep operational timing separate from deterministic replay identity. Sanitized response metadata, nullable usage, served model, finish reason, exact transport taxonomy and separate timing observations are archived and replayed. Success and failed-request telemetry passed `run_automation-025003-85`; subsequent model lane/executor regression passed `run_automation-153635-26`. No real model calls were made. |
| 291 | **open** | Instrumentation review recommendation: measure knowledge transfer on held-out function families under equal budgets, with explicit training/evaluation corpus membership, negative evidence and repeat-failure avoidance. Extend corpus generation beyond its single gate snapshot through an explicit immutable contributing-run set. Preserve all historical and excluded records. |
| 292 | **in progress** | Instrumentation review recommendation: expose the recipient/lane funnel with separate applicability, execution, generation, deduplication, evaluation, improvement, oracle and knowledge-reuse counts. Public run/status now project ordinary outcomes plus coordinator and request-bound worker measurements, including evaluation prefixes left by failed workers. The September 6 live run exposed hidden worker measurements and false inapplicability; #295 records the correction. Full production acceptance and the knowledge-transfer experiment remain open; data-segment instrumentation remains under #277. [September 14 funnel-counter acceptance: `search_funnel` now reports per-row and across-lane `lineage_reused` knowledge-reuse counts (candidates with a parent materialized by a different lane; unknown parents ignored, never guessed) with exact per-row identities, covered by helper unit tests plus factory-run integration proving zero-lineage baselines, zero-pending-oracle attribution at the oracle handoff, and ledger-recomputed identity agreement on the five-provider run. Evaluator suite 16/16. The knowledge-transfer experiment itself (also #291) stays open behind held-out multi-run design.] |
| 293 | **in progress** | Owner sequencing correction, 2026-09-05: model expansion before systemic automated matching was the wrong priority. The model activation portion of #283 is explicitly deferred; its earlier public-dispatch fixture passed with mocked transport in `run_automation-153906-26` and does not prove production completion. Preserve that implementation evidence, refuse new model-lane creation, and finish concrete programmatic wiring and execution first. The factory now connects idiom drafts and measured corpus/lineage inputs through an explicit indexed runtime identity, but no live corpus-backed idiom execution is claimed yet. Normal run and status output now project the lane funnel from actual archived outcomes and ledger measurements; live repository execution is in progress. |
| 294 | **done** | Live programmatic execution of `us:ST/RNZ1:SpikesApplyDamage` in `programmatic-wiring-20260905` exposed two production failures: synchronous creation exceeded the connector transport timeout although its child published a manifest, and preserved-candidate discovery selected RNO2 translation units by function name before evaluation aborted on missing `rno2.h`. Creation now uses the existing background-job authority. Explicit foreign-recipient candidates are rejected, candidate compilation receives the recipient overlay include directory, and diagnosed preprocessor failures become failed evaluations. The corrected factory/status/connector path passed `run_automation-030323-6501`; the live successor `programmatic-wiring-20260906` is being executed. Neither run is claimed as a checksum match. [September 7 closure: the successor-run wording above is historical. Background creation and recipient isolation were exercised through the later #298/#300/#296 public runs and actual automatic landing.] |
| 295 | **done** | The completed `programmatic-wiring-20260906` run retained 39 ledger events but its first funnel hid permuter evaluations behind runner failures and a 64 KiB response limit. The worker now measures the seed before mutation, preserves failed-process evaluation prefixes, accepts bounded full preprocessed translation units, and emits execution failure separately from inapplicability. GCC 2.6 undeclared-identifier diagnostics also required recognition without the word error. Harvesting now preserves unknown overlay conditionals; the live seed had lost its pointer-initialization branch. Instrumented transplant now calls the existing automatic preparation path with actual compiled-consumer context and destination translation units, and its manifest binds donor object/dependency/assembly/index inputs. Focused proof passed `run_automation-033358-6501`; public successor `programmatic-transplant-20260906` is in progress. No checksum match is claimed. [September 7 closure: the successor-run wording above is historical. Preparation and ordinary failed-worker prefix fixes were exercised through #300/#296. This does not close every interrupted-provider boundary under #287 or full production instrumentation acceptance under #292.] |
| 296 | **done** | Public factory-created searches do not yet bind `full_oracle` or reconstruct the existing `permuter_supervisor._instrumented_landing` callback, so an isolated zero cannot complete automated full-build landing. Wiring must preserve exact candidate translation-unit support declarations, journal/BuildLock recovery, checksum authority, durable terminal publication and queue method notes. A callable legacy landing function is not proof that the factory-to-oracle path exists. This is a systemic matching gap and precedes model activation. The explicit `land_matches` factory policy now binds and reconstructs automatic landing, archives exact before/after source, reserves the recipient, and persists oracle completion before recoverable queue reporting. Focused factory and interrupted-landing checks passed in `run_automation-173712-62`; live `programmatic-autoland-20260906` applied the real transplant, verified 113/113 and preserved all prior queue notes while reporting `matched`. Start job `search_start_instrumented-173900-62` then exposed an unsupported final stop enum; the existing `oracle_candidate_found` stop now carries that boundary, and `search_resume_instrumented-222643-62` closed the recorded attempt without rebuilding. Consolidated job `run_automation-222750-62` passed 103/105, exposing status-document drift and two preserved seeds with the newly resolved function declaration. Versioned seed repair retained all five generations, refreshed both queue method notes, and the corrected seed, status and supervisor checks passed 3/3 in `run_automation-223243-62`. The existing full oracle independently confirmed 113/113 after the live landing. |
| 297 | **done** | Corrected static production-audit false positives exposed by consolidated job `run_automation-033634-6501` (104/105 suites passed). Literal factory registration loops, canonical run-root reconstruction and helper-based provider revalidation are now recognized without executing Python; explicit factory lane refusals still prevent closure. This retracts the stale assertion that all nine generated/provider lanes lack dispatch. The focused audit passed `run_automation-034244-6501`, retains the missing m2c factory and deferred model admission findings, and remains a reachability report rather than proof of live qualification. |
| 298 | **done** | Corrected transplant factory capture after the first public creation completed without execution. The prior binding scanned removed depfiles but omitted the actual Ninja dependency database; it now binds `.ninja_deps`, `build.ninja`, their absence, and source freshness timestamps. Directory-entry validation removes repeated ancestor resolution while retaining symlink refusal. Focused factory proof passed `run_automation-091549-62`; corrected public creation `search_create_instrumented-091613-62` succeeded. That run exposed the separate preparation bug resolved in #300. Both original manifests remain evidence. |
| 299 | **done** | Concrete programmatic chaining gap: factory permuter providers freeze a renderer or preserved-file seed before deterministic lanes execute. Later task ancestry lists earlier candidates, but `PermuterLaneProvider._input_for` still returns that original frozen seed. A new compiling transplant therefore does not automatically become the next mutation input. Add a durable task-bound seed handoff selected from actual evaluated candidates, including exact source and parent measurement provenance, provider reconstruction and recovery validation. Listing candidates as task parents is not proof they were consumed. [September 14 implementation: task-local inputs now select exact source from successful nonzero ordinary measurements in the scheduled ledger prefix, preserving the immutable factory input map. Canonical handoff artifacts bind task, provider, candidate and measurement; supervisor dispatch and recovery rederive the same selection, and ordinary output records consumed-seed provenance. Concrete worker fixtures proved different measured source reached base.c, survived interruption after handoff publication and retained parent ancestry. Equal-score selection is deterministic; historical-archive/current-dispatch guards passed. Consolidated validation and landing remain pending. No live matching was run.] [September 14 completion: consolidated run_automation-072503-8358 passed 100/108; shared fixture, archive filename-hash and audit adapter-flow corrections passed all eight affected suites in run_automation-124252-15237 and run_automation-124319-15237. All 108 suites are accounted for. Concrete factory-to-worker execution, exact source preservation, deterministic tie selection, interrupted reconstruction, explicit fallback, ordinary candidate ancestry, corrupt/conflicting handoff refusal and historical archive/current-dispatch guards are verified. The previously published switch runtime remains compatible; no donor or renderer inputs changed, so no replacement runtime is needed. No live matching was run.] |
...
| 302 | **in progress** | Cross-platform function matching remains unqualified in production. On September 6, neither the canonical donor-snapshot store nor the indexed-runtime store existed. The four-platform scanner, immutable donor index, query and indexed adapter implementations therefore do not establish actual US/HD/PSPEU/Saturn matching coverage. Publish real pinned snapshots and a complete function index, measure cross-platform query results, and execute their target-context candidates through the ordinary matching path. This implements the existing production-indexed-runtime plan; queue indexing and the legacy US twin snapshot are not acceptance evidence. [September 8 implementation: published pinned source/configuration snapshots and runtime 1097fb7a8a2fa36f5d8da2afa121a37f6ca6be3440278ca7c690d82cb0f11fec with 3,634 donor functions: US 1,750, HD 99, PSPEU 1,440, Saturn 345. Fixed target-ID/platform/path filtering, target-bound semantic claims, canonical gate recovery paths, real C parser failures and host-dependent publication ordering. Publication completed in search_publish_indexed_runtime-134710-4693. Conditional-source preprocessing, assembly-backed structural coverage and broader target rendering remain implementation gaps. No new matching run or candidate landing was started; end-to-end execution is deferred by the owner.] [September 8 conditional-source implementation: source-only scanning now resolves known US/HD/PSP platform branches before include and declaration closure, preserves source offsets and original immutable snapshots, and records its projection policy. Unknown active conditions, platform redefinitions and comment-hidden splices remain exclusions; no Saturn platform facts are fabricated. Connector and factory reject non-US targets before run creation. Fixture integration covers pinned acquisition through projected donor scanning. General consumer preprocessing, assembly-backed structure and broader US rendering remain open. Runtime 1097fb7a remains the previously published generation; these scanner changes apply to the next explicit publication. Consolidated run_automation-190811-13708 passed 107/107 suites, including connector checks; no live matching or candidate landing was performed.] [September 9 leaf-rendering implementation: the US renderer now lowers bounded 32-bit scratch-register dataflow, bit operations, signed/unsigned comparisons, forward branches and joins with correct branch and return delay slots. Pinned donor scalar declarations reach the indexed renderer as per-function semantic facts; target prototypes retain priority. Synthesis no longer extracts branch-local returns as unconditional expressions. Compiled host fixtures cover both branch outcomes and word boundaries, actual PSX compilation passed, and reconstructed indexed adapters produce ordinary lane results. Consolidated run_automation-000027-67 passed 107/107 suites. General preprocessing, assembly-backed donor structure, calls, memory, loops and broader target declaration context remain open; no live matching was run.] [September 9 publication: successor runtime e00ea6151374973f5a3f85a9f1abc45e4803e7cb7379b72996e3ed9672ed9922 completed and verified in search_publish_indexed_runtime-000334-3464 using the same pinned revisions. It contains 6,798 source donors: US 3,046, HD 595, PSPEU 2,812, Saturn 345. Preprocessing exclusions are US 71, HD 17, PSPEU 121 and Saturn 11. This supersedes runtime 1097fb7a for current renderer bindings while preserving that archive; all donor assembly counts remain zero. Publication is implementation evidence, not live matching acceptance.] [September 9 US-context implementation: factory creation now archives the canonical US translation unit and its private, compiler-bound preprocessed context. Captured declarations feed indexed rendering, deterministic seeds and bounded synthesis, including reconstructed callbacks. Recovery rederives facts offline and refuses corrupt context; capture and dispatch reject source/header drift. Conservative parsing retains missing, unsupported and ambiguous declarations explicitly. Actual US preprocessing, fixture provider consumption and archive recovery passed; consolidated run_automation-080048-67 passed 107/107 suites. Calls, memory, loops, general donor preprocessing, assembly-backed donor coverage and #299 candidate chaining remain open. No live matching or candidate landing was performed.] [September 9 context publication: runtime b3bc0c140edd24376ac2f706f4634ef3812aa4757fd88e6c4a5807752017ce0c completed normal archive verification in search_publish_indexed_runtime-080352-3309. The same pinned revisions retain 6,798 source donors (US 3,046, HD 595, PSPEU 2,812, Saturn 345), with zero assembly-backed donors. This successor binds the context-aware renderer; prior generations remain preserved.] [September 9 direct-call implementation: US target rendering now supports bounded scalar direct calls and owned word stack saves/restores, with call side effects executed once, delay-slot arguments, caller-clobber and argument-home invalidation, and restored frame/register/return-address checks. Callee signatures are derived from archived US context in factory seeds and indexed target reconstruction; no donor supplies call ABI. Host behavior fixtures, actual US compilation, offline reconstructed permuter seeds and ordinary indexed-lane candidates/provenance passed focused checks. The jal-before-slot return-address timing case is regression-covered. Pointer/aggregate/wide and indirect/tail calls, heap/member memory, loops, general donor preprocessing, assembly-backed donor coverage and #299 chaining remain open; no live matching or landing was run.] [September 9 direct-call validation: consolidated run_automation-132141-68 passed 107/107 suites in 151.5 seconds. Runtime binding publication and the fresh pre-push oracle follow; this is fixture/implementation evidence, not live matching acceptance.] [September 12 completion confirmation: search_publish_indexed_runtime-132436-3331 succeeded and verified runtime fefddf5a3c55f371d3a934a18f46cc145bd9da10a3d8757d8f7fceb4f58dfb56. The same pinned source revisions retain 6,798 donors (US 3,046, HD 595, PSPEU 2,812, Saturn 345), with zero assembly-backed donors. This successor binds direct-call rendering; previous generations remain archived. Implementation files are unchanged from the passing consolidated run.] [September 12-13 US pointer/member implementation: archived US type layouts now drive byte/halfword/word named loads and stores, signed/unsigned extension, bounded fixed-array members, scalar pointer indexing, typed pointer moves/saves, compatible direct-call arguments and null/equality branches. Explicit load temporaries preserve alias and call ordering. Factory seeds carry archived US type declarations, and indexed reconstruction uses the same context projection. Every parser dependency is bound in factory tools and the aggregate runtime renderer identity, including supervisor reconstruction checks. Host behavior fixtures, standalone actual US compilation with Entity size/pointer/member-offset assertions, offline seed recovery and ordinary indexed receipts passed. Consolidated run_automation-213151-103 passed 106/108; the two inventory/documentation failures were corrected and both passed in run_automation-085527-50. Pointer-valued members/returns, pointer arithmetic, dynamic indexing, loops, ambiguous union selection, broader donor preprocessing, assembly-backed donor coverage and #299 chaining remain open. No live matching or candidate landing was performed.] [September 13 pointer/member publication: search_publish_indexed_runtime-085705-964 completed normal archive verification and published runtime e4365ab897be363d0c9ba6f1c72bc9dbc38af45613d21d749f9500b67b5e7dd7 in 609.1 seconds. The same pinned revisions retain 6,798 source donors (US 3,046, HD 595, PSPEU 2,812, Saturn 345), with zero assembly-backed donors. This successor binds US memory rendering and every concrete type-parser dependency; earlier generations remain archived.] [September 13 pointer-value implementation: canonical archived US pointer graphs now support pointer-valued named members, chained dereferences, compatible typed stores, call results and returns, null values, pointer arrays and bounded constant whole-element strides. Factory seed creation and indexed reconstruction include target/callee pointer return types and use the same immutable context. Host behavior and refusal fixtures, actual US Entity parent-chain/pointer-return compilation, offline seed recovery and ordinary indexed receipts passed. Consolidated run_automation-142725-50 passed 108/108 suites. Prototype-scope tags remain opaque outside their declaration; host helper names avoid the confirmed libc advance compatibility-symbol collision. Dynamic indexing, member-address derivation, loops, ambiguous union selection, broader donor preprocessing, assembly-backed donor coverage and #299 chaining remain open. No live matching or candidate landing was performed.] [September 13 pointer-value publication: search_publish_indexed_runtime-143035-14524 completed normal archive verification in 579.7 seconds and published runtime e635bbebfe7f97e51fb416963fa755d2566badb21d4c4cb15ff5f99099d1ea19. The same pinned revisions retain 6,798 source donors (US 3,046, HD 595, PSPEU 2,812, Saturn 345), with zero assembly-backed donors. This successor binds the expanded pointer renderer and context projection; prior generations remain archived.] [September 14 scoping, renderer stays as-is tonight: measured retrieval against a real published runtime is the next #302 evidence, as a read-only implementation check (bind the archived generation, query real US recipients per platform, tally matched/empty/incompatible/ambiguous/stale; no queue, no lanes, no landing). Assembly-backed donor structure, loops, dynamic indexing, member-address derivation and broader donor preprocessing remain the rendering gaps after that measurement. Runtime `99ded73d` (6,798 donors, `nonmatchings/search-evidence/indexed-runtimes/`) is the archived measurement target.] [September 14 measured retrieval, read-only: all 379 todo recipients queried symbol-only against `99ded73d` (US 3046, HD 595, PSPEU 2812, Saturn 345 donors): 74 matched, 63 ambiguous, 242 empty, zero incompatible/stale; donor hits pspeu 69, us 62, hd 2, saturn 0. Structural 40-record subsample (8 without stubs): 3 matched, 9 ambiguous, 20 empty, all 6 hits exact_symbol_path; donor hits pspeu 3, us 2, hd 1, saturn 0. Findings: Saturn's 345 entries never retrieve (unsupported layouts, as documented); HD barely retrieves; ambiguity dominates where same names collide across versions. First queries measured ~6/min through per-call binding; binding once per the production adapter pattern removed that cost. Runtime verification alone costs ~15 min over this mount (186MB re-read per process). No queue, lanes, builds, or writes were part of this lane.] [September 14 round 2, full unmatched scope (549 = 379 todo + 123 escalated + 41 deferred + 6 near): matched 81 (todo 74, escalated 3, deferred 4, near 0), ambiguous 76, empty 392, zero incompatible/stale; donor hits pspeu 75, us 68, hd 3, saturn 0. Non-todo retrieval is thin (7 matched outside todo), confirming the queue-status-blind donor index still leaves escalated/deferred work structurally uncovered. Ambiguity examples: EntityBossDoors (todo), EntityUnkId1B (escalated), EntityGaibon (deferred), EntitySealedDoor (near).] [September 14 control-flow census, read-only over 1751 nonmatching files (719 functions, 1032 data): straight 74, straight-with-call 63, forward-branch 250, switch-dispatch (jtbl) 127, true-indirect 146, loop 59. Only 1 loop file fits the 64-instruction renderer bound, so loop support alone unlocks ~1 function without bigger-function support. The 146 true-indirect files are dominated by the g_api-pointer-call idiom (lui/lw/jalr, e.g. func_us_801AF31C); the renderer has zero g_api awareness today (no match in search_target_renderer.py) and refuses all jalr, so that idiom plus archived API-signature evidence is the largest well-defined rendering gap. Dynamic indexing proxy (sll+addu feeding memory): 444/719, over-approximating (fixed-point math shares the shape). Member-address derivation is not textually separable from supported constant-offset access; it needs dataflow measurement, still open.] [September 15 g_api-pointer-call implementation: renderer now lowers the lui/lw/jalr API idiom with paired-hi/lo value tracking, load-delay and clobber checks, plus target-owned api_declarations projected from archived context (target_pointer_declaration and _api_member_names, 64-member bound). Call ABI covers s16/u16/s8/u8/char/short/bool/PrimitiveType and unnamed helper parameters with synthesized positional names; pointer returns stay refused. Census over 177 true-indirect jalr files finds paired g_api hi/lo in all 177 and zero without: AllocPrimitives 114, PlaySfx 48, CheckCollision 27. Fixture proof renders void, scalar, s16/enum and unnamed shapes; test_search_target_renderer 30/30 and test_search_compile_driver 9/9 pass, evaluator rerun 16/16 after one flaky lineage count in the consolidated run. Only 9 of the 177 fit the 64-instruction bound and each still carries other unsupported shapes, so no full file flips yet; member-layout and larger-function support remain open.] [September 15 variable renderer bounds: RendererLimits exposes max_instructions, path_budget, max_expression and max_body with validated hard ceilings (512/4096/16384/262144) and canonical defaults equal to the previous constants, so production archived replay is unchanged. Same step adds the negu leaf instruction (8 measured occurrences, trapping neg stays refused) and skips semantically empty function directives (size/ent/end/frame/mask/fmask/loc, 1 per file). Attribution over the 9 small files shows the remaining blockers are global-data relocations (72 lines across g_PrimBuf, EInits, g_Entities and player/GPU fields, a data-search epic) while constant-offset member access already works through layouts; dynamic indexing and globals stay explicitly out. Fixture proof: exact 64/65 boundary, 73-instruction render under raised caps with gcc semantics, tiny-budget termination, invalid-limit refusals, negu gcc semantics, directive skip with data refusal, plus real-PSX compiles; test_search_target_renderer 37/37, test_search_compile_driver 10/10, consolidated 109/109.] [September 15 flip census over the 177: 156 fit 512 instructions, 140 loop-free, 101 with handled mnemonics only. End-to-end through real TU context (20 with declared symbols): exactly 1 renders at default limits, func_us_801B7CC4_from_no1, and its draft PSX-compiles to a 23-row near-match of the 24-instruction target (deltas: move/addu spelling, li/ori spelling, sh base a0-vs-s0 same address, jal delay nop-vs-addu); not landed, matching phase deferred. Raised limits add 0 because every larger file carries global-data relocations. 62 files have a TU but no symbol declaration there (header-defined bodies such as EntitySpikesParts in e_spikes.h need header-scope capture, scoped separately). Remaining blocks with counts: loops 37, mult/div 49/18 files, break 18, unaligned lwl/lwr/swl/swr 7 each, GTE coprocessor lines, 423 non-API relocation symbols led by g_PrimBuf 128. Unnamed-callee plus narrow-ABI admission (DestroyEntity, u8/s16) is what unlocked the single flip.] [September 15 multiply/divide leaf family: mult/multu/div/divu latch HI/LO as a deferred triple beside the register file (branch-safe, call-persistent, overwritten by later mult/div); mflo/mfhi lower low word, signed/unsigned high word and quotient/remainder with no C signed-overflow UB; break 6/7 modeled as no-ops after proving plain C division compiles to the identical div/bnez/break/mflo guard shape; no HI/LO latency rule after measuring 169 adjacent mult-mf pairs as genuine compiler output. Fixtures: gcc semantics for low/high/quotient/remainder, 7 refusal shapes, PSX shape reproduction asserting mult/div/break/mflo in real output; renderer 44/44, compile-driver 12/12. Census effect: end-to-end pool tried 20 to 25 with still 1 flip; attribution over the covered pool shows all 123 remaining refusals blocked solely by global-data relocation lines, zero by instruction shapes.] [September 15 sibling-capture measurement: 99 structurally ready files lack a TU declaration; same-overlay textual hits cluster in e_init.c prototypes, but the unsup histogram over the 99 shows zero render-ready files (minimum 2 unsupported lines, all global-data relocs), so sibling/header capture alone flips 0. Envelope design recorded (same-overlay scope, grep-then-preprocess with overlay cache, single-signature rule, protocol v2 with sibling artifacts mirroring the TU input/preprocessed pair) and deferred behind the data epic on measured binding order: data-relocs bind first. Concrete triggers: DestroyEntity-style unnamed prototypes and EntitySpikesParts-class header bodies resolve through the same rule when data lands.] [September 15 data census: 176 of 177 carry non-API relocs; D_* 407 refs with 389 same-overlay resolved in the existing data index (18 D_800xxxxx main-exe refs out of scope), g_* 423 refs needing header types, PLAYER_/RIC_ absolute symbols plus small tables as a linker-symbol remainder. Only 3 files are D_*-only and all 3 are also declaration-less, so slices compose rather than flip alone. Dominant D_* use is address computation (lui/addiu %lo pairs, ~490) feeding pointer arguments, not scalar loads, so binding means pointer-value integration with unit-derived pointee types, built together with sibling capture as one epic; loops after.] [September 16 D_* address binding: renderer lowers lui %hi(D) plus addiu %lo(D) to pointer values with paired-hi tracking, extern emission and constant-expression folding for splat immediates; caller threads data_declarations; fixture proof renders array address and scalar address-of with missing/unpaired/clobbered refusals; test_search_target_renderer 47/47, compile-driver 14/14, run-factory 56/56, layout 9/9. g_* header types and linker remainder stay open; no live matching was run.] [September 16 g_* address binding: renderer-neutral global_declarations projected from exact context excluding g_api, plus lui/addiu pointer lowering with paired-hi tracking and extern emission; provider seeds strip the derived view; fixture proof renders global array address with missing/unpaired refusals; test_search_target_renderer 50/50, compile-driver 15/15, run-factory 56/56. Linker-symbol remainder stays open; no live matching was run.] [September 16 linker census, read-only: PLAYER_ %hi refs exceed 2000 (capped count, truncated), RIC_ 455, jtbl 1268 already handled via dispatch recovery, g_CastleFlags plus-offset expressions 1964 which the current exact-pair matcher refuses. PLAYER_/RIC_ shapes mix scalar halfword loads (lhu/lh) and lui/addiu address forms; their types live in hand-written externs (e.g. extern u16 RIC_step) and absolute symbol files, not TU context. Scoped next design: absolute scalar-load lowering with width-derived externs plus offset-expression admission, measured against the 177-file pool before implementation.] [September 16 linker remainder implementation: linker_declarations projected from exact context with width-derived u8/s8/u16/s16/u32 externs at render time including conflict refusal; lui/addiu address lowering for declared PLAYER_/RIC_ plus g_* offset addresses via whole-element advance; scalar lb/lbu/lh/lhu/lw lowering for absolute symbols and byte-array offsets with load-delay and extern emission; stores and non-byte offset widths stay refused. Fixture proof: width-derived lhu, declared address-of, offset address and byte-index loads with store/width-conflict/unpaired refusals; test_search_target_renderer 56/56, compile-driver 16/16, run-factory 56/56, layout 9/9. End-to-end remeasurement against the 177-file pool stays open; no live matching was run.] [September 16 pool remeasurement, read-only via measure_data_effect: 283 nonmatchings files pair jalr with g_api hi/lo under the comment-tolerant filter (Sept pool was 177 under the earlier true-indirect filter); exactly 1 renders at default limits through real TU plus sibling context, the known func_us_801B7CC4_from_no1, so the three landed slices hold with zero regressions and zero errors over the pool. Relocation histogram: D_* 231, g_* 278, linker 75, jtbl 106, other 32; 63 files undeclared in the owning TU. Flip blockers shift to loops, switch dispatch, header-scope bodies and unclassified relocs; loops stay next per epic order. No live matching was run.] [September 16 raised-limits tally via --limits raised: same single render (func_us_801B7CC4_from_no1); raised bounds add 0 flips even with data slices landed. Size histogram over 282 unrendered: 8 fit 64, 25 fit 65-128, 105 fit 129-256, 75 fit 257-512, 69 exceed 512. Size blocks at most 69 files while 213 within 512 refuse on shape, so loop and switch support outrank bigger bounds. No live matching was run.] [September 16 loop-shape census via has_loop_shape mirroring the renderer loop rule: 206 of 283 pool files carry a backward branch; size split 1 fit 64, 7 fit 65-128, 76 fit 129-256, 63 fit 257-512, 59 exceed 512. Loop support within current ceilings addresses at most 147 files; the 59 above 512 need bigger bounds with new termination proof first. Rendered still 1. Scoped loop design next: bounded counted loops with exact trip counts, refusing unbounded and delay-slot-entry shapes. No live matching was run.] [September 16 latch-form census: 171 multi-latch, 30 single conditional-latch (do-while candidates), 5 single unconditional-latch (while candidates) of 206 loop files. Single-region lowering covers at most 35 files; the nested-vs-sequential split rides with the loop implementation slice. Rendered still 1. No live matching was run.] [September 16 do-while loop lowering: admitted straight-line regions with one backward conditional latch lower to do-while with materialized loop-carried scalars and pointers, load-first exemption, shared loop_regions admission serving lowering and census, all definition sites routed through _define. Fixture proof: counter, pointer-walk sum and store loops render host-exact; 7 refusal shapes covered. Renderer 69/69, measure 10/10. Pool effect unmeasured pending shard reruns after one vanished full run; no live matching was run.] [September 16 sharded loop census, 4 shards over 283 files: 134 files admit at least one do-while region yet only the known flip renders. Refusal file-votes: branch-in-loop 117, call-in-loop 88, while-latch 55, undeclared TU 63, nested-loop 26, outside-entry 12, barred-op 8, zero errors. Order for full loop support: while-latch emission, in-loop branch joins, per-iteration calls, then nesting and entries; switch dispatch and bigger ceilings after. No live matching was run.] [September 16 while-form lowering: unconditional backward latches with one forward exit lower to do-while-break or test-first while via exact condition negation, unified with do-while admission; while-latch refusals fully absorbed into 33 admitted while regions. Fixture proof: sentinel-sum, zero-trip and early-exit loops host-exact. Renderer 73/73, measure 10/10. Sharded pool over final code: 142 files admit regions yet still 1 render; refusal file-votes branch-in-loop 116, call-in-loop 91, undeclared 63, nested-loop 31, outside-entry 12, barred-op 8, multi-exit 3, zero errors. Next: in-loop branch joins, then per-iteration calls. No live matching was run.] [September 16 in-loop branch joins: bare-if spans with exact taken/fall merging lower through _lower_segment, atomic regions route by start, join temps pre-initialized so taken-arm state matches; refusal split into nested-loop, call-in-loop, return-in-loop, crossing-branch and branch-in-loop. Fixture proof: divergent-value join host-exact plus scratch-temp coverage; exact-join-over-marker and stuck-state cases refused. Renderer 75/75. Sharded pool at default limits over live queue: pool 260 active with 23 stale matched skipped, rendered 0 new, admitted-blocked 6 declared plus 16 size; refusal file-votes branch-in-loop 49, call-in-loop 112, multi-exit 27, nested-loop 26, outside-entry 14, barred-op 11, return-in-loop 3, while-no-exit 2, crossing-branch 1, zero errors. Calls outrank branches now. No live matching was run.] [September 16 loop/call outcome document: docs/loop-call-outcome-2026-09-16.md records landed slices with commits and fixture proof, sharded pool tallies, the two broken call-lowering invariants with the revert, and the ordered next slices; dashboard diagnostics carry pool-smoke entries and the full selftest gate passes 110/110. No live matching was run.] |
...
| 305 | **open** | Expand scorer calibration beyond the first two real function families, add independent lineage groups and measure an improvement before recommending new default weights. The #304 production tuner and later-run consumption are complete, but its three bounded configurations tied and are exploratory evidence only. Keep holdout separate from selection and preserve the initial reports. [September 14 attempt, stays open: ran the exact production tuner path for run `tuning-expansion-20260914` and it refused correctly at source capture. Of all archived candidates only `programmatic-weight-adoption-20260907` is terminally completed and admissible; `programmatic-autoland-20260906` stopped at `oracle_candidate_found` with one incomplete task, `programmatic-weight-permuter-20260907` and `programmatic-transplant-20260906-c` fail ledger verification, and the wiring/transplant runs have no canonical manifests. A third independent family therefore needs new measurement executions, which the September 8 implementation-only order defers. No weights changed; the partial run dir remains local evidence.] |
| 306 | **done** | September 13 source assessment of PSXRecomp 85cd26f05c44999731f6b3320fb8a871fba68e9b and recomp-ui cb7e54b41b6d75a6233de0083914e270f3902f4c: selected MIPS analysis and regression scenarios may support #302; native-runtime C does not replace matching, and launcher integration is deferred to an explicit playable-port objective. Recorded implementation evidence, scope limits and PolyForm Noncommercial versus MIT licensing in docs/psxrecomp-assessment-2026-09-13.md. No external builds, benchmark, live matching or candidate landing were performed. |
| 307 | **done** | September 13 owner-approved MIPS regression cases added to the existing renderer/layout suites: 60 paired load-hazard controls, 60 compiled delay-slot/alias/word-boundary checks, pointer-address dependency, merge-load and control-slot refusals, and archived indirect-dispatch/delay-slot-entry refusal provenance with a forward-branch positive control. Expected semantics were independently checked against the IDT R30xx manual; no PSXRecomp code was copied. Focused run_automation-194108-30 passed 2/2; consolidated run_automation-194125-30 passed 108/108. No production defect was exposed, no renderer capability was added, and no live matching or candidate landing ran. Plan and detailed outcome: docs/superpowers/plans/2026-09-13-mips-regression-cases.md. |

| 308 | **done** | Independently rederived US jump-table recovery from local SOTN assembly and MIPS semantics, without incorporating PSXRecomp code or dependencies. Bounded C89 switches pass compiled behavior, actual PSX compilation, archived candidate and provider reconstruction, ordinary indexed receipts, table-tamper and dependency-drift tests. Corrected historical-gate compatibility exposed by real publication; post-fix consolidated run_automation-005237-30 passed 108/108. Published and verified successor runtime sha256:99ded73daabda8e042761428d4318b98a2ed6312062478b17542c53ee8b8b935 with the same 6,798 donors. Publication took approximately 53m22s; sampled WSL filesystem waits and repeated source materialization are recorded for future optimization. Read-only generated-assembly coverage recovered 110 dispatches in 98 files, but none fit the current whole-function instruction limit; no new renderable live function or match is claimed. Derivation, validation evidence and the required post-commit landing procedure: docs/superpowers/plans/2026-09-13-independent-switch-recovery.md. |

| 309 | **done** | September 14 owner tool-route correction: the lead acknowledged normal-tool authorization but continued connector filesystem writes. Persisted AGENTS.md constraint 25, explicitly reserving normal non-Git tool liberties for Astra and Fable only, requiring the owner-selected file-tool route, and retaining connector Git, canonical WSL builds and the exact pre-push gate. Other models retain the existing restrictions. |

| 310 | **done** | September 15 review of Muse Spark's loop/call implementation: retracted the #302 outcome diagnosis that loop carriers must survive volatile clobbering or need a new call-result kind. Separated carrier storage from register validity, reused ordinary direct/API call ABI lowering, added bounded live-in discovery and backedge validation, and repaired predicate/delay-slot ordering, zero-trip effects, loop-load NameError, branch-local join initialization and initialized live-outs. Focused renderer, actual PSX compile, archived/indexed receipt and factory reconstruction suites pass. Consolidated 110/110, default-bound pool measurement and raised-limits pool remeasurement are complete with zero renders and zero errors; #302 production qualification and live matching remain deferred. Landed and pushed as commit 382904a0a with oracle 113/113. Detailed evidence: docs/loop-call-outcome-2026-09-16.md. |

| 311 | **done** | Multi-exit loop joins for #302: admit forward exits that share one continuation immediately after the loop, lower each exit to its own predicate snapshot, delay slot and break, and merge only bindings valid on every exit path. Split continuations still refuse multi-exit, nonlocal shared exits still refuse nonlocal-exit, and join-plus-multi composition stays refused as branch-in-loop until each shape is proven separately. Fixture proof is host-executed do-break and while-form loops plus region admission tests. Default-bound pool remeasurement is complete with zero errors: multi-exit 27 to 17, join-plus-multi composition deferred to branch-in-loop, structural admission 164 with zero renders. |
...
| 317 | **done** | Bridge gate fixes at owner direction: registered test_http_bridge.py in the run_automation allowlist. RETRACTION: the exit-zero starlette skip was wrong; the suite now re-execs under automation/mcp/.venv (sys.prefix check, because both venvs symlink to the same /usr/bin/python3.12) and fails closed if that venv is missing. Full suite passes under both the MCP venv and the root .venv via re-exec. |

| 318 | **done** | Sibling and header declaration capture: capture archives same-overlay .c scopes whenever a referenced member misses from the owning unit, plus raw header scopes for missing functions and members; render-time projection falls back with single-signature consensus and refuses static header definitions. Fixture proof covers global, linker and callee fallback, cross-sibling ambiguity, static poisoning, capture archiving and archive replay. Default and raised pool remeasurements are complete with zero errors and zero renders: the missing symbols are engine externs, todo-dependencies and uncapped cases. |

| 319 | **done** | Successor indexed-runtime publication binding loops, calls, mult, else joins, data slices and sibling scopes: same gate programmatic-tuning-context-20260907 and pins eb178335006be58f274d0c8157addae1461325bc as runtime 99ded73d. Publication job search_publish_indexed_runtime-215152-77098 succeeded with runtime a8faa2271286a99b65da51f3cd3622db98d597446f30ecdfbf4f066652073cb2 holding 6,798 donors in 57 minutes; archive verification job search_verify_indexed_runtime-224912-77098 returned valid in 11 minutes. No queue or matching activity was part of this. |

| 320 | **done** | Exit-shape classification (C0): per-reason specimen lists in the tally plus manual review of four shape-blocked files show no nested-continuation splits. Exit targets are shared function epilogues which cannot nest, remaining branch-in-loop votes are epilogue jumps and continue-to-latch shapes, and clean loops in those files are blocked by calls and data. Else-if ladders (C1) are a no-go; splits and nonlocal exits stay refused. |

| 321 | **in progress** | Missing-external tally over todo, deferred and escalated notes (job run_automation-004641-43751): 543 records, 235 naming missing symbols. Headliners are engine functions AnimateEntity 123 and SetStep 73, linker data PLAYER_posX_i_hi 55 and g_Entities_160 42, then UnkCollisionFunc3 26 and SetSubStep 19. Deferred and escalated carry nearly none of it; the load sits in todo. Curation proposal for the engine function shortlist is with the owner. |

| 322 | **done** | Shared stage-header scope: same-overlay header scans missed stage-common helpers like AnimateEntity and SetStep that live in src/st/*.h, so the header mention scan now covers those top-level shared headers with PSP variants excluded, and verification accepts that root. Fixture proof covers cross-overlay adoption, PSP exclusion and path-forgery refusal. Full-pool tally and consolidated validation to be recorded before landing. |

| 323 | **done** | Fleet model probe for Muse Spark: internal id is muse-spark-1.3-contributor-free, live and free in the CLI catalog. Zen-direct HTTP refuses it with 401 not supported. CLI serves tiny prompts but returns zero bytes on a real 11k-char decomp prompt at both 60s and 300s timeouts, the documented dead-call shape. Effort xhigh exists nowhere in the worker, probe, fleet or dashboard surfaces; Zen honors enable_thinking false only with measured default reasoning none. Verdict: do not make it the fleet default; mimo-v2.5-free stands. Probe fixes landed alongside: lowercase debug log flag restoring CLI probing, and a backend flag for worker-path probes. |

| 324 | **done** | Dashboard fleet picker carries muse-spark-1.3-contributor-free as an explicitly warned non-default: Zen refuses the id and the CLI returns zero bytes on real prompts, so it stays selectable for reproduction and out of every default path. [2026-09-16 note: the Zen refusal was chat-completions shaped; Spark serves /v1/responses with HTTP 200 for low and xhigh, see #325.] |
| 325 | **done** | Unblocked the spark xhigh fleet path at owner direction. fleet_start rejected reasoning xhigh while dashboard effort 2 already mapped to xhigh and worker Responses passes it through, so the knob read as configured and did nothing. Gate now accepts xhigh as Responses-only with the 2026-09-16 measurement recorded; the MCP wrapper gained and forwards no_char_limits (previously silently dropped on the MCP path); dashboard stale two-value comments corrected; dry-run regression pinned in test_connector_surfaces. Bounded probe first: 3 declaration-complete RNZ1 records on spark low with full asm executed end to end (Responses generation, one quality-gate scaffolding verdict moving one record todo to escalated, one HTTP 429), queue otherwise intact per matched audit 434 present 0 lost. Live xhigh still needs a connector restart to load the edited server. |
| 326 | **done** | Added the no-timeout fleet option at owner direction after Spark xhigh attempts died client-side on 30K-char prompts. Timed probe: trivial xhigh prompt answers HTTP 200 with first byte at 112s, so large prompts outthink any fixed socket deadline. `fleet_start(no_timeout=True)` sets GEN_TIMEOUT=3600, and every call site takes min() with the function budget, so the budget stays the only real deadline. Wired through the MCP wrapper and the dashboard picker with dry-run regression pins. No worker change needed. |
| 327 | **done** | Corrected no_timeout the same day: GEN_TIMEOUT alone does not bind, the 90s ATTEMPT_BUDGET does, via budget_left=min() at every call site. That is what killed the spark xhigh attempts, and the first version would have changed nothing. no_timeout now sets GEN_TIMEOUT=3600 and ATTEMPT_BUDGET=3600 together across the connector, wrapper, dashboard, and regression pin. |
| 329 | **done** | Fixed the prompt-vs-gate scaffolding contradiction and the silent review-gate skips. The prompt told workers to copy DECLARATIONS lines above the function while _candidate_function_only rejects any file-scope residual, burning attempts on obedience. Prompt now says match types without re-emitting and emit only the single definition. review_gate keeps identical return behavior but module-load failure logs loud and distinct, and all skip lines carry the exception message. test_review_gate green. |
| 330 | **done** | Forwarded full_power and no_output_token_limit through the MCP wrapper. The dashboard switch never reached workers over MCP: the wrapper accepted neither flag, so full-power requests ran fully capped. Same silent-drop class as the earlier gates, now pinned by a forwarding check. |
| 331 | **done** | Prompt variant selection by offset need, not just name affinity. Verified the failure first: entity 0x9E/0xA0 are ET_GurkhaHammer.length/parent (union member GH_Props), which affinity can never match for numeric-named functions, while the gate queries all 461 variants. ext_variants_for now appends variants with a named field at each raw-accessed ext offset after the affinity picks. Unnamed and placeholder fields never select. Pinned in test_draft_cleaning, suite green. |
| 332 | **done** | Broadened offset backfill to demanded offsets after the raw-only trigger proved insufficient: drafts show unkNN, not raw views, so the first version stayed silent where needed. Placeholder and unk/pad names still never select. |
| 333 | **done** | Apply-time declaration injection now covers retained data. Prompt-time lookup already resolved D_us labels, but the injector that writes before the build only grepped C sources, so every model candidate using retained data died undeclared. Unresolved D_ names now fall back to the retained-data index with exact-overlay matching, since the same spelling names different objects per overlay. Ambiguous and absent labels still refuse. Pinned with the live failing symbol D_us_8018063C, suite green. |
| 334 | **done** | Probe discipline: scheduler --small-first claims by instruction count for bounded runs instead of coverage rank, which preferred declaration-rich giants and re-fed them after requeues. Deferred-last and blocked-last still outrank it, unknown rows degrade to rank order, and the flag is off by default. Wired through worker env, fleet_start, wrapper, and dashboard with ordering and forwarding pins. max_functions documented as per-worker everywhere after a 4-cap batch ran 7 functions. |
| 335 | **done** | Variant disambiguation: when several named variants cover one demanded offset, the prompt now lists up to two and names the ambiguity with entity-type selection guidance instead of first-in-file luck. Factored as a unit-testable selector with fabricated-index pins. |
...
| 340 | **done** | Finished the interrupted score-zero landing run_automation-180623-87758: 14 upstream-harvest v4 bodies recorded with proof and landed across 10 source files (BO0 x4, RBO7, RBO8 x2, RCEN, RCHI, RNZ1, RNO4, RNO1 x3) at 113/113; 4 rejected with evidence (three DrawLaserRing share one systematic 8-byte overshoot; 801B9028 diverges from the shared header). Provenance correction re-reported all 14 with METHOD=UPSTREAM-HARVEST after they first classified as permuter/unknown; transplant.py now propagates the candidate METHOD into every landing note with self-test pins. |
