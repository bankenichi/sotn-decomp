...
| 215 | **open** | Repair the deferred-triage verdict authority exposed by #98 before any `--requeue`: 19 proven post-repair exhaustions are classified as `degraded-search` because their durable verdict predates `SEED_CURRENT` or records a controlled search with ordinary nonzero mutation compile errors. Preserve the two genuinely degraded records, replace prose inference with durable structured evidence, and add a live-pool regression proving the corrected 33 exhausted / 2 degraded split |
