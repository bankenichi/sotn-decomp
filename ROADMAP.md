...
| 239 | **partial, 2/3** | Close the complete three-record near pool in deterministic order: `us:ST/RCEN:func_801904B8`, `us:ST/RNO0:func_us_801C7F24`, then `us:BOSS/BO0:func_us_801B001C`. RCEN is done at 81/81 from the same-basename CEN sibling and target-proven signed adaptation. RNO0 is done at 81/81 from the RARE stone-skull sibling: saved-body scoring now restores declaration context, receipt landing filters exact function identity before limits, full-build mismatches capture pre-restore structural and relocation evidence, and the vendored permuter now scores branch destinations and parses bare hexadecimal jump addends. That instrumentation isolated the required explicit debug-guard `break` at branch word `0x4807c`; the corrected body verified RNO0 SHA-1 `125003c7fe5d3510fddee22d4f5e5390fe507acf`. Continue only with `us:BOSS/BO0:func_us_801B001C` |
