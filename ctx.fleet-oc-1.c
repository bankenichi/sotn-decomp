...
static void BO6_CheckBladeDashInput() {
    u16 step = g_Entities[64].step;
    if ((step == 1 || step == 2 || g_Entities[64].step == 3 || step == 4 || step == 5) &&
...
void BO6_RicStepWalk(void) {
    if (BO6_RicCheckInput(0x305C) == 0) {
        ...
        if (BO6_RicCheckFacing() == 0) {
            BO6_RicSetStand(0);
        } else if (g_Entities[64].step_s == 0) {
...
void func_us_801B77D8(void) {
    if (g_Entities[64].posX.i.hi - g_Entities[0].posX.i.hi <= 0) {
        g_Entities[64].entityRoomIndex = 0;
    } else {
        g_Entities[64].entityRoomIndex = 1;
    }
}
;
;
