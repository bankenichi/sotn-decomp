...
// HARVESTED from upstream/master src/boss/bo0/e_secrets.c.
//
// TWO RENAMES WERE NEEDED, and both are deliberate.
//
// 1. The two static functions. Upstream calls them func_us_801B59C4 and
//    func_us_801B6794, which are their addresses in ST/NO2 -- upstream's copy
//    was extracted from the NO2 version and kept NO2's names. In BO0 they live
//    at 0x801AB9EC and 0x801AC73C, which is what this fork's queue and its
//    INCLUDE_ASM stubs already called them. Keeping upstream's names here would
//    have meant two symbols asserting addresses they are nowhere near, in the
//    one overlay where the file is actually compiled.
//
// 2. The six file-scope arrays, for the same reason and by the same constant.
//    Every upstream name is exactly +0x300 from its true BO0 address
//    (D_us_80180D48 -> 0x80180A48, and so on down). That the offset is uniform
//    across all six is the evidence the arithmetic is right, not a guess.
//
// The preprocessor branches are upstream's, untouched. bo0.h defines
// BOSS_IS_BO0 so the #ifndef arms compile out here, but this fork already
// shares e_prisoner.h, e_marionette.h and e_chair.h between BO0 and the
// stages, so the arms are what makes this file usable that way later. Deleting
// them would be a one-way loss and would end the ability to diff this against
// upstream.
...
static void func_us_801AC73C(Primitive* prim) {
    s32 x, y;
...
static u16 D_us_80180B14[] = {
    0x086, 0x087, 0x088, 0x089, 0x0F6, 0x0F7, 0x0F8, 0x0F9,
};
static u16 D_us_80180B24[][8] = {
