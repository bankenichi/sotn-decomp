...
// Layout scan cursors used by the horizontal/vertical entity-creation scanners below.
// Storage still lives in the undecompiled data/53EB8.bss.s blob, so only reference it.
extern u16* g_LayoutObjHorizontal;
extern u16* g_LayoutObjVertical;

// Direction flags for the horizontal/vertical scan cursors above. Storage
// lives in the undecompiled data/53EB8.bss.s blob.
...
// Scans g_LayoutObjHorizontal for the next element with an x position to the
// right of posX, or the end of the list.
...
// Scans g_LayoutObjHorizontal for the next element with an x position below
// posX, or the beginning of the list.
...
// Creates entities between the previous horizontal position and the passed
// in x position toward the right.
...
// Scans g_LayoutObjVertical for the next element with a y position above
// posY, or the end of the list.
...
// Scans g_LayoutObjVertical for the next element with a y position below
// posY, or the beginning of the list.
...
// Creates entities between the previous vertical position and the passed
// in y position going up.
...
// Sets up layout arrays and initializes stage state to initial values for
// the given room.
...
// Creates any new entities that may need to be created based on a change
// in the scroll delta.
