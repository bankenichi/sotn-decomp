...
static s32 VandalSwordDrawAlastor(s32 arg0, s32 arg1) {
    ...
    if (!(g_Timer & 3)) {
        if (g_CurrentEntity->ext.vandalSword.alastorClut & 0x4000) {
            g_CurrentEntity->ext.vandalSword.alastorClut--;
            if ((g_CurrentEntity->ext.vandalSword.alastorClut) < 0x4000) {
                g_CurrentEntity->ext.vandalSword.alastorClut = 0;
            }
        } else {
            g_CurrentEntity->ext.vandalSword.alastorClut++;
            if ((g_CurrentEntity->ext.vandalSword.alastorClut) > 15) {
                g_CurrentEntity->ext.vandalSword.alastorClut = 0x400F;
            }
        }
    }
    ...
    case 1:
        temp_s2 = ((g_CurrentEntity->ext.vandalSword.alastorBrightness) *
                       abs(rsin(g_CurrentEntity->ext.vandalSword.alastorAngle)) >>
                   0xC);
        prim->clut = ((g_CurrentEntity->ext.vandalSword.alastorClut & 0xF) + 0x210);
        ...
        if (g_CurrentEntity->facingLeft) {
            prim->x0 = prim->x2 +=
                FLT_TO_I(rcos(g_CurrentEntity->ext.vandalSword.alastorAngle) * 2);
            prim->x1 = prim->x3 -=
                FLT_TO_I(rcos(g_CurrentEntity->ext.vandalSword.alastorAngle) * 2);
        } else {
            prim->x0 = prim->x2 +=
