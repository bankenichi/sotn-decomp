...
WHAT IT REGENERATES
    Three fenced blocks, delimited by HTML comment markers so the surrounding
...
def _generated_block_findings(
    ...
    for name, begin, end, block in expected:
        ...
        if begin_count != 1 or end_count != 1:
            bad.append(
                f"README.md {name} markers are not unique: begin="
                f"{begin_count}, end={end_count}; expected one of each"
            )
            continue
        actual = _inclusive_block(text, begin, end)
        if actual is None:
            bad.append(f"README.md {name} markers are out of order")
        elif actual != block.replace("\r\n", "\n"):
            bad.append(f"README.md {name} block is stale; run "
                       "automation/readme_status.py --write")
    return bad


def drift_generated_readme() -> list[str]:
    """Generated README blocks must equal their live generators exactly."""
    ...
    return _generated_block_findings(text, expected)
...
DRIFT_CHECKS = (
    ...
    ("generated README", drift_generated_readme),
)
...
def splice(text: str, begin: str, end: str, block: str) -> str:
    ...
    if begin_count != 1 or end_count != 1:
        raise LookupError(
            f"expected exactly one {begin} and one {end} in README.md; "
...
def main() -> int:
    ...
    try:
        ...
        text = splice(text, COMPLETION_BEGIN, COMPLETION_END, cb)
...
def self_test() -> int:
    ...
    print("\ngenerated README drift is an exact marker-delimited comparison")
    generated = (
