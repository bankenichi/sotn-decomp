...
def fleet_start(workers: int = 4, max_functions: int = 0) -> dict:
...
def _fleet_pids_alive() -> list[int]:
    """Live worker PIDs, from the pid file, falling back to a process scan.

    The pid file can be missing or stale (a timed-out launch, a manual start,
    a crash). Without the fallback the fleet becomes invisible: status shows
    nothing and stop cannot reap anything, while workers keep running and
    holding claims.
    """
    alive: list[int] = []
    f = REPO / FLEET_PIDS
    ...
    if not alive:
        r = subprocess.run(["bash", "-lc", "pgrep -f worker_direct || true"],
                           cwd=str(REPO), capture_output=True, text=True, timeout=30)
        alive = [int(x) for x in r.stdout.split() if x.isdigit()]
    # NEVER trust pgrep alone. Its own bash wrapper carries the pattern on its
    # command line, so a naive match returned init and kernel pids (1, 2, 5,
    # 10) here. Confirm each pid really is one of our workers by reading its
    # cmdline; anything else would mean fleet_stop killing arbitrary processes.
    confirmed: list[int] = []
    for pid in alive:
        try:
            ...
            continue
        text = cmd.replace(b"\0", b" ").decode("utf-8", errors="replace")
        if "worker_direct.py" in text and "python" in text and "pgrep" not in text:
...
def fleet_stop() -> dict:
    ...
    try:
        ...
        pass
    return {"action": "fleet_stop", "stopped": alive,
            "reclaim": r.stdout.strip(), "note": "claims released, lock cleared"}
