/* PERMUTER SEED -- compiled and linked, bytes differ.
   record : us:BOSS/BO0:func_us_801B5D6C
   attempt: 2/4
   ...
   import : python3 tools/decomp-permuter/import.py <this file> asm/us/boss/bo0/nonmatchings/2D26C/func_us_801B5D6C.s
...
s32 func_us_801B5D6C(s16 *arg0, s16 *arg1, s16 arg2) {
    s32 result = 1;
    s32 i;
    s16 threshold = arg2;  // $t3 holds sign-extended arg2
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (diff < threshold) {
            ...
            result = 0;
            if (val1 < orig_val0) {
                arg0[0] = orig_val0 - threshold;
            }
            // Reload after potential modification
            val0 = arg0[0];
            val1 = arg1[0];
            if (val0 < val1) {
                arg0[0] = threshold + val0;
            }
        }

        arg0 += 2;  // +4 bytes = 2 s16 elements
        arg1 += 1;  // +2 bytes = 1 s16 element
    }
