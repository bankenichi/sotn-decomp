/* PERMUTER SEED -- deterministic isolated-score candidate.
   ...
   score  : 10
   receipt: nonmatchings/.adapt-scores/20260824-235137-62298-353941/EntitySurfacingWater/adapt-score.json
   producer: compiled-donor transplant, no model
   content: WHOLE FILE (isolated adaptable draft)
   origin : src/st/rnz1/unk_37BF8.c
   asm    : asm/us/st/rnz1/nonmatchings/unk_37BF8/EntitySurfacingWater.s
...
void EntitySurfacingWater(Entity* self) {
    ...
    case 0:
        ...
        params = self->params;
        if (!(params & 0x8000)) {
            g_api_PlaySfxVolPan(g_WaterSounds[1], 0x7F, temp_s3);
        }
        ...
        self->ext.waterEffects.topY.i.hi =
            -(D_us_80181EA8[self->params & 0xFF] + 12 + (rand() & 1));
    ...
    i = self->velocityX;
    if (i != 0) {
        x = g_Tilemap.width - D_us_80181060[self->ext.waterEffects.unk88] -
            D_us_80181060[self->ext.waterEffects.unk88 + 1];
        if (i < 0) {
            ...
            x += (D_us_80181060[self->ext.waterEffects.unk88 + 1] - 6 -
