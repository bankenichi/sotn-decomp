...
void func_us_801BCB9C(Entity* self) {
    ...
    if (self->step != 1) {
        if (self->step < 2) {
            ...
            InitializeEntity((u16*)&D_us_80180C60);
            self->velocityX = 0x20000;
            if (self->params == 0) {
                self->velocityX = 0x10000;
            }
            self->velocityY = 0xFFFE0000; /* -0x20000 initial upward? */
            ...
            if (AnimateEntity((u8*)D_us_8018224C, self) != 0) {
                return;
            }
            for (i = 0; i < 7; i++) {
                newShard = AllocEntity(g_Entities_224, &g_Entities_224[32]);
                if (newShard != NULL) {
                    CreateEntityFromEntity(0x46, self, newShard);
                    newShard->params = Random() & 7;
                }
            }
