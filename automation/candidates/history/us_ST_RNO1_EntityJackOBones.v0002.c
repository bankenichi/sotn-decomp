/* PERMUTER SEED -- deterministic isolated-score candidate.
   ...
   score  : 10
   receipt: nonmatchings/.adapt-scores/20260824-233943-62298-670873/EntityJackOBones-2/adapt-score.json
   producer: compiled-donor transplant, no model
   content: WHOLE FILE (isolated adaptable draft)
   origin : src/st/rno1/unk_35378.c
   asm    : asm/us/st/rno1/nonmatchings/unk_35378/EntityJackOBones.s
...
extern EInit g_EInitJackOBones;
extern u8 D_us_80181C24[13];
extern u8 D_us_80181C34[13];
extern u8 D_us_80181C44[24];
extern u8 D_us_80181C5C[10];
extern u8 D_us_80181C68[12];
extern u8 D_us_80181C84[8];
extern s32 D_us_80181C8C[7];
extern s32 D_us_80181CA8[7];
extern s16 D_us_80181CC4[8];
extern s16 D_us_80181CD4[8];
extern u8 D_us_80181CE4[2][4];
extern s16 D_us_80181CEC[8];
extern s16 D_us_80181D04[6];
void TryThrow(void);
...
void EntityJackOBones(Entity* self) {
    ...
    case JACKO_INIT:
        ...
        break;
    case JACKO_1:
        if (UnkCollisionFunc3(D_us_80181CEC) == 0) {
            break;
        }
        self->step++;
        break;
    case JACKO_WALK_FWD:
        if (AnimateEntity(D_us_80181C24, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
    ...
    case JACKO_WALK_BACK:
        if (AnimateEntity(D_us_80181C34, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
        ...
        break;
    case JACKO_THROW:
        var_s2 = AnimateEntity(D_us_80181C44, self);
        ...
        if (!var_s2) {
            ...
            self->ext.jackoBones.throwTimer =
                D_us_80181CE4[self->params & 1][var_s2];
            break;
        }
    ...
    case JACKO_JUMP:
        ...
        case JACKO_JUMP_WINDUP:
            if (!(AnimateEntity(D_us_80181C5C, self) & 1)) {
        ...
        case JACKO_JUMP_MIDAIR:
            if (UnkCollisionFunc3(D_us_80181CEC)) {
                self->step_s++;
            }
            CheckFieldCollision(D_us_80181D04, 2);
            break;
        case JACKO_JUMP_LANDING:
            if (AnimateEntity(D_us_80181C68, self) == 0) {
                SetStep(JACKO_WALK_BACK);
            }
        }
        break;
    case JACKO_DEAD:
        ...
        for (i = 0; i < 6; i++) {
            ...
            other->ext.jackoBones.deathPartLife = D_us_80181C84[i];
            if (self->facingLeft) {
                other->posX.i.hi -= D_us_80181CC4[i];
            } else {
                other->posX.i.hi += D_us_80181CC4[i];
            }
            other->posY.i.hi += D_us_80181CD4[i];
            other->velocityX = D_us_80181C8C[i];
            other->velocityY = D_us_80181CA8[i];
        }
        DestroyEntity(self);
        break;
    }
