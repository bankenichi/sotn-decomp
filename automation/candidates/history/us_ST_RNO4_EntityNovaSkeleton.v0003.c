/* PERMUTER SEED -- deterministic isolated-score candidate.
   ...
   score  : 10
   receipt: nonmatchings/.adapt-scores/20260824-234650-62298-188647/EntityNovaSkeleton-2/adapt-score.json
   producer: compiled-donor transplant, no model
   content: WHOLE FILE (isolated adaptable draft)
   origin : src/st/rno4/unk_58A30.c
   asm    : asm/us/st/rno4/nonmatchings/unk_58A30/EntityNovaSkeleton.s
...
extern EInit g_EInitNovaSkeleton;
extern s16 D_us_801829F0[8];
extern u8 D_us_80182A08[13];
extern u8 D_us_80182A18[13];
extern u8 D_us_80182A40[11];
extern u8 D_us_80182A4C[66];
extern u8 D_us_80182A90[48];
extern u8 D_us_80182AD0[8];
extern s32 D_us_80182AD8[8];
extern s32 D_us_80182AF8[8];
extern s16 D_us_80182B18[8];
extern s16 D_us_80182B28[8];
extern u8 D_us_80182B38[8];
void TryShoot(void);
...
void EntityNovaSkeleton(Entity* self) {
    ...
    case NOVA_INIT:
        ...
        break;
    case NOVA_1:
        if (UnkCollisionFunc3(D_us_801829F0) == 0) {
            break;
        }
        SetStep(NOVA_IDLE);
        break;
    case NOVA_IDLE:
        ...
        AnimateEntity(&D_us_80182A40, self);
        if (GetDistanceToPlayerX() < 0x70) {
            SetStep(NOVA_WALK_BACK);
        }
        break;
    case NOVA_WALK_FWD:
        if (AnimateEntity(&D_us_80182A08, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
        ...
        break;
    case NOVA_WALK_BACK:
        if (AnimateEntity(&D_us_80182A18, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
    ...
    case NOVA_5:
        break;
    case NOVA_CHARGE:
        if (AnimateEntity(&D_us_80182A4C, self) == 0) {
    ...
    case NOVA_SHOOT:
        ...
        DrawLaserRing();
        if (!AnimateEntity(&D_us_80182A90, self)) {
            prim = self->ext.nova.prim;
            prim->drawMode = DRAW_HIDE;
            var_s4 = ++self->ext.nova.laserTimerIndex & 7;
            self->ext.nova.cooldown = D_us_80182B38[var_s4];
            SetStep(NOVA_WALK_BACK);
        }
        break;
    case NOVA_DEAD:
        for (i = 0; i < 6; i++) {
            ...
            other->facingLeft = self->facingLeft;
            other->params = i;
            other->ext.nova.deathPartLife = D_us_80182AD0[i];
            if (self->facingLeft) {
                other->posX.i.hi -= D_us_80182B18[i];
            } else {
                other->posX.i.hi += D_us_80182B18[i];
            }
            other->posY.i.hi += D_us_80182B28[i];
            other->velocityX = D_us_80182AD8[i];
            other->velocityY = D_us_80182AF8[i];
        }
