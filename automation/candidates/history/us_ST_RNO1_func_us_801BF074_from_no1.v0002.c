/* PERMUTER SEED -- deterministic isolated-score candidate.
   ...
   score  : 5
   receipt: nonmatchings/.adapt-scores/20260824-234201-62298-630742/func_us_801BF074_from_no1/adapt-score.json
   producer: compiled-donor transplant, no model
   content: WHOLE FILE (isolated adaptable draft)
   origin : src/st/rno1/unk_26178.c
   asm    : asm/us/st/rno1/nonmatchings/unk_26178/func_us_801BF074_from_no1.s
...
#define FLAG_HAS_PRIMS 8388608
#define PRIM_GT4 4
extern s32 D_us_80180CDC[5][2];
extern s16 D_us_80180D04[6];
extern s16 D_us_80180D10[6];
extern u8 D_us_80180D1C[14];
extern EInit g_EInitParticle;
extern struct Entity;
void DestroyEntity(Entity*);
...
void func_us_801BF074_from_no1(Entity* self) {
    ...
    case 1:
        ...
        if (primIndex != -1) {
            ...
            prim->next->x1 = self->posX.i.hi;
            prim->next->y0 = self->posY.i.hi;
            LOH(prim->next->r2) = D_us_80180D04[self->params];
        ...
        self->velocityX = D_us_80180CDC[self->params][0];
        self->velocityY = D_us_80180CDC[self->params][1];
        self->step++;
        break;

    case 2:
        prim = self->ext.segmentedBreakableWall.prim;
        LOH(prim->next->tpage) += D_us_80180D10[self->params];
        prim->next->x1 = self->posX.i.hi;
        prim->next->y0 = self->posY.i.hi;
        UnkPrimHelper(prim);
        if (!AnimateEntity(D_us_80180D1C, self)) {
        ...
        g_api_CheckCollision(posX, posY, &collider, 0);
