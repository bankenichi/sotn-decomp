/* PERMUTER SEED -- deterministic isolated-score candidate.
   ...
   score  : 10
   receipt: nonmatchings/.adapt-scores/20260824-234008-62298-660493/EntityNovaSkeleton-2/adapt-score.json
   producer: compiled-donor transplant, no model
   content: WHOLE FILE (isolated adaptable draft)
   origin : src/st/rno1/unk_35378.c
   asm    : asm/us/st/rno1/nonmatchings/unk_35378/EntityNovaSkeleton.s
...
extern EInit g_EInitNovaSkeleton;
extern s16 D_us_80181D10[8];
extern u8 D_us_80181D28[13];
extern u8 D_us_80181D38[13];
extern u8 D_us_80181D60[11];
extern u8 D_us_80181D6C[66];
extern u8 D_us_80181DB0[48];
extern u8 D_us_80181DF0[8];
extern s32 D_us_80181DF8[8];
extern s32 D_us_80181E18[8];
extern s16 D_us_80181E38[8];
extern s16 D_us_80181E48[8];
extern u8 D_us_80181E58[8];
void TryShoot(void);
...
void EntityNovaSkeleton(Entity* self) {
    ...
    case NOVA_INIT:
        ...
        break;
    case NOVA_1:
        if (UnkCollisionFunc3(D_us_80181D10) == 0) {
            break;
        }
        SetStep(NOVA_IDLE);
        break;
    case NOVA_IDLE:
        ...
        AnimateEntity(&D_us_80181D60, self);
        if (GetDistanceToPlayerX() < 0x70) {
            SetStep(NOVA_WALK_BACK);
        }
        break;
    case NOVA_WALK_FWD:
        if (AnimateEntity(&D_us_80181D28, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
        ...
        break;
    case NOVA_WALK_BACK:
        if (AnimateEntity(&D_us_80181D38, self) == 0) {
            self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        }
    ...
    case NOVA_5:
        break;
    case NOVA_CHARGE:
        if (AnimateEntity(&D_us_80181D6C, self) == 0) {
    ...
    case NOVA_SHOOT:
        ...
        if (!AnimateEntity(&D_us_80181DB0, self)) {
            ...
            self->ext.nova.cooldown = D_us_80181E58[var_s4];
            SetStep(NOVA_WALK_BACK);
        }
        break;
    case NOVA_DEAD:
        for (i = 0; i < 6; i++) {
            ...
            other->facingLeft = self->facingLeft;
            other->params = i;
            other->ext.nova.deathPartLife = D_us_80181DF0[i];
            if (self->facingLeft) {
                other->posX.i.hi -= D_us_80181E38[i];
            } else {
                other->posX.i.hi += D_us_80181E38[i];
            }
            other->posY.i.hi += D_us_80181E48[i];
            other->velocityX = D_us_80181DF8[i];
            other->velocityY = D_us_80181E18[i];
        }
