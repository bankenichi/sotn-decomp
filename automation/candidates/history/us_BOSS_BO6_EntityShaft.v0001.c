/* PERMUTER SEED -- compiled and linked, bytes differ.
   record : us:BOSS/BO6:EntityShaft
   attempt: 3/4
   ...
   asm    : asm/us/boss/bo6/nonmatchings/us_3E79C/EntityShaft.s
...
extern int BO6_RicGetFreeEntity();
...
/* Shaft boss intro/fade-in entity: handles his appearance, scaling, and fade-out */
void EntityShaft(Entity* self) {
    ...
    case 1:
        ...
        if (self->opacity >= 0x31) {
            self->step++;
            CreateEntityFromCurrentEntity(0x17, &g_Entities[200]);
            g_Entities[200].params = 3;
            ((s16*)&self->ext)[0] = 0x100;
        }
        /* fallthrough to common tail */
        self->posY.val += rsin(((s16*)&self->ext)[1]) * 4;
        ((s16*)&self->ext)[1] += 0x20;
        break;
    ...
    case 3:
        temp_u = self->scaleX - 0x20;
        self->scaleX = temp_u;
        if ((s16)temp_u < 0x10) {
            self->scaleX = 0x10;
        }
        temp_u = self->scaleY + 0x40;
        self->scaleY = temp_u;
        if ((s16)temp_u >= 0x801) {
            self->scaleY = 0x800;
        }
    ...
    case 4:
        self->opacity += 0xFD; /* -3 as unsigned */
        if (self->opacity < 4) {
            DestroyEntity(self);
        }
        break;
