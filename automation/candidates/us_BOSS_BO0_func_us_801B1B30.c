/* PERMUTER SEED -- compiled and linked, bytes differ.
   record : us:BOSS/BO0:func_us_801B1B30
   attempt: 1/4
...
--- build tail ---
  ...
  ✅
   ...
   import : python3 tools/decomp-permuter/import.py <this file> asm/us/boss/bo0/nonmatchings/2D26C/func_us_801B1B30.s
...
s32 func_us_801B1B30(Entity* entity) {
    // Switch on hitFlags to perform different spawn checks
    u8 hitFlags = entity->hitFlags;
    Entity* parent = entity->parent;
    s32 result;

    switch (hitFlags) {
        case 0:
            // First spawn check: check two regions, sum must be 2
            result = func_us_801B171C(entity, -0x180, 0, 0x20);
            result += func_us_801B171C(parent, 0x80, 0x280, 0x20);
            if (result == 2) {
                // Advance to next state
                entity->hitFlags = hitFlags + 1;
            }
            return 0;
        case 1:
            // Second spawn check: first region must be non-zero
            result = func_us_801B171C(entity, -0x200, -0x100, 0x40);
            func_us_801B171C(parent, -0x100, 0x280, 0x40);
            if (result != 0) {
                // Advance to next state
                entity->hitFlags = hitFlags + 1;
            }
            return 0;
        case 2:
            // Third spawn check: first region must be non-zero
            result = func_us_801B171C(entity, -0x280, -0x200, 0x60);
            func_us_801B171C(parent, -0x180, 0x300, 0x40);
            if (result != 0) {
                return 1; // Indicate successful spawn?
            }
            return 0;
        default:
            return 0;
