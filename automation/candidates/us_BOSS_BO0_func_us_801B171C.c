/* PERMUTER SEED -- compiled and linked, bytes differ.
   record : us:BOSS/BO0:func_us_801B171C
   attempt: 4/4
   ...
   import : python3 tools/decomp-permuter/import.py <this file> asm/us/boss/bo0/nonmatchings/2D26C/func_us_801B171C.s
...
/* Moves entity toward target on X and Y axes by a step, returns 1 if both axes reached target exactly. */
s32 func_us_801B171C(Entity* entity, s16 targetX, s16 targetY, s16 step) {
        u16 moved;
        s16 absDiff;
        s16 target;
    s16* posX = (s16*)((s8*)entity + 0xC);
    s16* velX = (s16*)((s8*)entity + 0xE);
    s16* posY = (s16*)((s8*)entity + 0x10);
    s16* velY = (s16*)((s8*)entity + 0x12);
    s16 currentX, diffX, currentY, diffY;
    moved = 0;
    
    /* Process X axis first */
    currentX = *posX;
    diffX = targetX - currentX;
    absDiff = diffX < 0 ? -diffX : diffX;
    ...
    /* Process Y axis second */
    currentY = *posY;
    diffY = targetY - currentY;
    absDiff = diffY < 0 ? -diffY : diffY;
