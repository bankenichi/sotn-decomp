/* PERMUTER SEED -- compiled and linked, bytes differ.
   record : us:BOSS/BO0:func_us_801B2044
   attempt: 3/4
...
--- build tail ---
  ...
  ✅
   ...
   import : python3 tools/decomp-permuter/import.py <this file> asm/us/boss/bo0/nonmatchings/2D26C/func_us_801B2044.s
...
// counter as a cheap delay-timer advance.
//
// The switch on zPriority dispatches:
//   0: short move (-0x400, 0x200), small advance (8)
//   1: short move (-0x400, -0x40), small advance (8)
//   default: long move (-0x300, -0x40), medium advance (0xC)
void func_us_801B2044(Entity *ent) {
    s32 a1;     // per-frame offset passed to helper
    s32 a2;     // per-frame duration passed to helper
    s32 a3;     // a1+a2 combined parameter passed to helper

    u8 zPriority = ent->zPriority;   // frame-counter / delay-timer value

    if (zPriority == 1) {
        // case 1: short offset, small advance
        a1 = -0x400;     // -1024
        a2 = -0x40;      // -64
        a3 = 8;
    } else if (zPriority >= 2) {
        // default (>=2): long offset, medium advance
        a1 = -0x300;     // -768
        a2 = -0x40;      // -64
        a3 = 0xC;        // 12
    } else {
        // case 0: short offset positive, small advance
        a1 = -0x400;     // -1024
        a2 = 0x200;      // 512
        a3 = 8;
    }

    // Helper applies the (a1, a2, a3) transform; nonzero return means advance.
    if (func_us_801B171C(ent, a1, a2, a3) != 0) {
        // Increment the frame counter stored at zPriority (0x24).
        // Loaded with lbu and stored with sb, so the value stays a u8.
        ent->zPriority = (u8)(ent->zPriority + 1);
    }
}
