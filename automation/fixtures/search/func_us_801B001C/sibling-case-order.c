void func_us_801B001C(Entity* self) {
    ...
    register s32 caseParams __asm__("$3");
    ...
    case 1:
        if (self->flags & 0x100) {
            caseParams = (u8)self->params;
            self->hitboxState = 0;
            self->step = caseParams + self->step;
        }
        return;
    ...
    case 8:
        ...
        self->velocityY = velocityY + 0x1000;
    ...
    case 9:
        ...
        velocityY = self->velocityY;
        self->velocityY = velocityY + 0x1800;
        if (func_us_801AD2F0(posX, posY) != 0) {
            self->entityId = 2;
