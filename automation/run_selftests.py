...
def run_one(path: Path, timeout: int) -> tuple[str, bool, float, str]:
    t0 = time.time()
    try:
        ...
        return path.name, r.returncode == 0, time.time() - t0, tail[:90]
    except subprocess.TimeoutExpired:
        return path.name, False, time.time() - t0, f"TIMED OUT after {timeout}s"
    except OSError as e:
        return path.name, False, time.time() - t0, f"{type(e).__name__}: {e}"
...
def main() -> int:
    ...
    print(f"{'suite':<34}{'':<4}{'secs':>6}  verdict")
    print("-" * 96)
    for name, ok, secs, tail in rows:
        if a.failed_only and ok:
            continue
        print(f"{name:<34}{'ok' if ok else 'FAIL':<4}{secs:>6.1f}  {tail}")
    print("-" * 96)
    print(f"{len(rows) - len(bad)}/{len(rows)} suites passed "
          f"in {time.time() - t0:.1f}s wall")
    if bad:
        print("\nFAILED: " + ", ".join(r[0] for r in bad))
    # dashboard self-test is not a test_*.py, so say so rather than implying
    # this covered everything.
    print("\nNot included here: `dashboard.py --self-test` and "
          "`empty_response_audit.py --self-test`, which live with their own "
          "modules. Both have their own buttons.")
